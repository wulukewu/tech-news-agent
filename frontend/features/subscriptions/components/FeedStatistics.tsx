  ...
  return (
    <div className="flex items-center gap-0.5">
      {Array.from({ length: 5 }).map((_, i) => (
        <Star
          key={i}
          className={`w-3 h-3 transition-all duration-300 hover:scale-[1.1] ${
            i < Math.floor(rating)
              ? 'fill-yellow-400 text-yellow-400 animate-pulse [animation-duration:3s]'
              : i < rating
          ...
          >
            <CardContent className="p-3">
              <div className="flex items-center gap-2">
                <Icon
                  className={`w-4 h-4 ${stat.color} transition-transform duration-300 hover:scale-[1.1] animate-pulse [animation-duration:3s]`}
                />
                <div className="flex-1 min-w-0">
                  ...
                  {stat.content ? (
                    <div className="mt-1 animate-in fade-in duration-300">{stat.content}</div>
                  ) : (
                    <div className="text-lg font-semibold transition-colors duration-300 hover:text-primary">
