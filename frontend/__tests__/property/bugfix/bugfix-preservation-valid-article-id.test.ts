...
describe('Preservation Property Tests - Valid article_id Operations', () => {
  ...
  it('should successfully update status with valid UUID article_id', async () => {
    ...
    expect(mockPatch).toHaveBeenCalledWith(`/api/reading-list/${validUuid}/status`, { status });
  ...
  it('should successfully update rating with valid UUID article_id', async () => {
    ...
    // Verify API was called with correct URL and payload
    expect(mockPatch).toHaveBeenCalledTimes(1);
    expect(mockPatch).toHaveBeenCalledWith(`/api/reading-list/${validUuid}/rating`, { rating });
    ...
    for (const status of statuses) {
      await fc.assert(
        fc.asyncProperty(fc.uuid(), async (validArticleId) => {
          ...
          try {
            const result = await updateReadingListStatus(validArticleId, status);
            ...
            if (url !== `/api/reading-list/${validArticleId}/status`) return false;
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async (validArticleId, rating) => {
          ...
          try {
            const result = await updateReadingListRating(validArticleId, rating);
            ...
            const [url, payload] = calls[0];
            if (url !== `/api/reading-list/${validArticleId}/rating`) return false;
  ...
  it('should preserve API call structure and contracts (property-based)', async () => {
    ...
    await fc.assert(
      fc.asyncProperty(fc.uuid(), async (validArticleId) => {
        ...
        if (mockPatch.mock.calls[0][0] !== `/api/reading-list/${validArticleId}/status`)
          return false;
        ...
        if (mockPatch.mock.calls.length !== 1) return false;
        if (mockPatch.mock.calls[0][0] !== `/api/reading-list/${validArticleId}/rating`)
          return false;
        ...
        if (mockDelete.mock.calls[0][0] !== `/api/reading-list/${validArticleId}`) return false;
