  ...
  it('should detect TypeScript compilation errors in unfixed code', () => {
    ...
    if (typeCheckExitCode !== 0) {
      ...
      const hasFeedCardError =
        typeCheckOutput.includes('isSubscribed') && typeCheckOutput.includes('FeedCard.tsx');
      ...
      console.log(`Jest DOM type definition error: ${hasJestDomError ? 'FOUND' : 'NOT FOUND'}`);
      ...
      if (hasJestDomError) {
        ...
        console.log('- Test files use: toBeInTheDocument(), toHaveAttribute(), etc.');
          ...
          try {
            ...
            if (filename === 'FeedCard.tsx') {
              ...
              if (!hasJestDomError && typeCheckOutput.includes(filename)) {
                console.log(`Expected Jest DOM error in ${filename} but not found`);
              }
