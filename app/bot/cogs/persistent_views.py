...
class PersistentReadLaterButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        try:
            ...
            # Save to reading list (database operation)
...
class PersistentMarkReadButton(discord.ui.Button):
    ...
    def __init__(self):
        super().__init__(
            ...
            label="✅ 標記已讀",
            custom_id="mark_read_persistent"
        )
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        try:
            ...
            # Update article status (database operation)
            supabase = SupabaseService()
...
class PersistentRatingSelect(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        try:
            ...
            # Update article rating (database operation)
            supabase = SupabaseService()
            await supabase.update_article_rating(discord_id, article_id, rating)
            
            # Send confirmation
            await interaction.followup.send(
                f"✅ 已評為 {stars}（{rating} 星）！", ephemeral=True
            )
...
class PersistentDeepDiveButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        try:
            ...
            # Fetch article from database (necessary after bot restart)
            supabase = SupabaseService()
            ...
            # Generate deep dive
            result = await LLMService().generate_deep_dive(article)
            if len(result) > 2000:
                result = result[:1997] + "..."
...
class PersistentInteractionView(discord.ui.View):
    ...
    def __init__(self):
        ...
        self.add_item(PersistentReadLaterButton())
        self.add_item(PersistentMarkReadButton())
        self.add_item(PersistentRatingSelect())
        self.add_item(PersistentDeepDiveButton())
