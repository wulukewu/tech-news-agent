...
def test_property_9_valid_cron_expressions_accepted(cron_expression):
    ...
    with patch("app.tasks.scheduler.settings") as mock_settings:
        ...
        assert len(jobs) >= 1, f"No jobs were added for valid CRON expression '{cron_expression}'"
        ...
        assert (
            job.name == "Background Article Fetch and Analysis"
...
def test_property_9_invalid_cron_expressions_rejected(cron_expression):
    ...
    with patch("app.tasks.scheduler.settings") as mock_settings:
        ...
        assert (
            "Invalid CRON expression" in error_message
        ), f"Error message should mention 'Invalid CRON expression', got: {error_message}"

        assert (
            cron_expression in error_message
...
def test_property_9_cron_trigger_creation(cron_expression):
    ...
    assert trigger is not None, f"CronTrigger should not be None for valid CRON '{cron_expression}'"
...
def test_property_9_cron_trigger_rejection(cron_expression):
    ...
    assert exc_info.value is not None, f"CronTrigger should reject invalid CRON '{cron_expression}'"
...
def test_property_9_scheduler_accepts_and_creates_job(cron_expression):
    ...
    with patch("app.tasks.scheduler.settings") as mock_settings:
        ...
        assert job is not None, f"Job should be created for valid CRON '{cron_expression}'"

        assert job.id == "background_fetch", f"Job ID should be 'background_fetch', got '{job.id}'"
