...
def test_property_1_user_deletion_cascades(test_supabase_client: Client, test_feed, discord_id):
    ...
    **Validates: Requirements 3.9**
    ...
    subscription_result = (
        ...
        .insert({"user_id": user_id, "feed_id": test_feed["id"]})
        .execute()
    )
    ...
    article_result = (
        ...
        .insert({"feed_id": test_feed["id"], "title": "Test Article", "url": article_url})
        .execute()
    )
    ...
    reading_list_check = (
        test_supabase_client.table("reading_list").select("*").eq("id", reading_list_id).execute()
    )
...
def test_property_2_feed_deletion_cascades(test_supabase_client: Client, feed_name, category):
    ...
    **Validates: Requirements 3.10**
    ...
    feed_result = (
        ...
        .insert({"name": feed_name, "url": feed_url, "category": category, "is_active": True})
        .execute()
    )
    ...
    article_1_result = (
        test_supabase_client.table("articles")
        .insert({"feed_id": feed_id, "title": "Test Article 1", "url": article_url_1})
        .execute()
    )
    ...
    article_2_result = (
        test_supabase_client.table("articles")
        .insert({"feed_id": feed_id, "title": "Test Article 2", "url": article_url_2})
        .execute()
    )
    ...
    article_1_check = (
        test_supabase_client.table("articles").select("*").eq("id", article_1_id).execute()
    )
    assert len(article_1_check.data) == 0, "Article 1 should be cascade deleted"

    article_2_check = (
        test_supabase_client.table("articles").select("*").eq("id", article_2_id).execute()
    )
...
def test_property_3_article_deletion_cascades(
    test_supabase_client: Client, test_user, test_feed, article_title
):
    """
    **Validates: Requirements 3.11**
    ...
    article_result = (
        test_supabase_client.table("articles")
        .insert({"feed_id": test_feed["id"], "title": article_title, "url": article_url})
        .execute()
    )
    ...
    reading_list_result = (
        test_supabase_client.table("reading_list")
        .insert({"user_id": test_user["id"], "article_id": article_id, "status": "Unread"})
        .execute()
    )
    reading_list_id = reading_list_result.data[0]["id"]
...
def test_property_4_discord_id_uniqueness(test_supabase_client: Client, discord_id):
    ...
    **Validates: Requirements 6.1**
    ...
    try:
        ...
        assert (
            "duplicate" in error_msg or "unique" in error_msg or "constraint" in error_msg
...
@given(dummy=st.just(None))  # No random data needed, using fixtures
@settings(
    max_examples=5, suppress_health_check=[HealthCheck.function_scoped_fixture], deadline=None
)
def test_property_5_subscription_uniqueness(
    test_supabase_client: Client, test_user, test_feed, dummy
):
    """
    **Validates: Requirements 6.4**
    ...
    try:
        test_supabase_client.table("user_subscriptions").insert(
            {"user_id": test_user["id"], "feed_id": test_feed["id"]}
        ).execute()
...
def test_property_6_reading_list_entry_uniqueness(
    test_supabase_client: Client, test_user, test_article, dummy
):
    """
    **Validates: Requirements 6.5**
    ...
    try:
        test_supabase_client.table("reading_list").insert(
            {"user_id": test_user["id"], "article_id": test_article["id"], "status": "Read"}
        ).execute()
    ...
    **Validates: Requirements 7.3**
    ...
    try:
        test_supabase_client.table("feeds").insert(
            {"name": f"{feed_name} 2", "url": feed_url, "category": category, "is_active": True}
        ).execute()
    ...
    **Validates: Requirements 7.4**
    ...
    try:
        test_supabase_client.table("articles").insert(
            {"feed_id": test_feed["id"], "title": f"{article_title} 2", "url": article_url}
        ).execute()
...
def test_property_9_shared_feed_references(test_supabase_client: Client, test_feed, num_users):
    ...
    **Validates: Requirements 7.5**
    ...
    for i in range(num_users):
        ...
        user_result = (
            test_supabase_client.table("users").insert({"discord_id": discord_id}).execute()
        )
    ...
    for subscription_id in subscription_ids:
        ...
        assert (
            subscription.data[0]["feed_id"] == test_feed["id"]
        ), "All subscriptions should reference the same feed_id"

    # Verify only one feed record exists with this URL
    feeds = test_supabase_client.table("feeds").select("*").eq("url", test_feed["url"]).execute()
...
def test_property_10_required_field_validation(test_supabase_client: Client, test_feed, dummy):
    ...
    **Validates: Requirements 9.3, 9.4, 9.5, 9.6, 9.7, 9.8**
    ...
    try:
        test_supabase_client.table("users").insert({"discord_id": None}).execute()
        pytest.fail("User insertion with NULL discord_id should have failed")
    except Exception as e:
        error_msg = str(e).lower()
        assert (
            "null" in error_msg or "not null" in error_msg or "violates" in error_msg
        ), f"Expected NOT NULL constraint error, got: {e}"

    # Test feeds.name NOT NULL
    try:
        test_supabase_client.table("feeds").insert(
            {"name": None, "url": f"https://test-{os.urandom(8).hex()}.com", "category": "Test"}
        ).execute()
    ...
    try:
        test_supabase_client.table("feeds").insert(
            {"name": "Test Feed", "url": None, "category": "Test"}
        ).execute()
    ...
    try:
        test_supabase_client.table("articles").insert(
            {"feed_id": test_feed["id"], "title": "Test Article", "url": None}
        ).execute()
...
def test_property_11_timestamp_auto_population(test_supabase_client: Client, test_feed, discord_id):
    ...
    **Validates: Requirements 8.1, 8.2, 8.3, 8.4, 8.6, 8.7**
    ...
    feed_result = (
        test_supabase_client.table("feeds")
        .insert({"name": "Test Feed", "url": feed_url, "category": "Test"})
        .execute()
    )
    feed = feed_result.data[0]
    ...
    subscription_result = (
        test_supabase_client.table("user_subscriptions")
        .insert({"user_id": user["id"], "feed_id": test_feed["id"]})
        .execute()
    )
    subscription = subscription_result.data[0]
    assert (
        subscription["subscribed_at"] is not None
    ), "user_subscriptions.subscribed_at should be auto-populated"
    ...
    assert (
        reading_list["updated_at"] is not None
    ), "reading_list.updated_at should be auto-populated"
...
def test_property_12_reading_list_status_validation(
    test_supabase_client: Client, test_user, test_article, status
):
    """
    **Validates: Requirements 9.1**
    ...
    try:
        test_supabase_client.table("reading_list").insert(
            {"user_id": test_user["id"], "article_id": test_article["id"], "status": status}
        ).execute()
        pytest.fail(f"Reading list insertion with invalid status '{status}' should have failed")
    except Exception as e:
        error_msg = str(e).lower()
        assert (
            "check" in error_msg or "constraint" in error_msg or "violat" in error_msg
...
@given(rating=st.integers(min_value=-1000, max_value=1000).filter(lambda r: r < 1 or r > 5))
@settings(
    max_examples=5, suppress_health_check=[HealthCheck.function_scoped_fixture], deadline=None
)
def test_property_13_rating_range_validation(
    test_supabase_client: Client, test_user, test_article, rating
):
    """
...
def test_property_14_embedding_null_tolerance(
    test_supabase_client: Client, test_feed, article_title
):
    """
...
def test_property_15_seed_script_active_flag(test_supabase_client: Client, dummy):
    ...
    **Validates: Requirements 4.7**
    ...
    for category in seed_categories:
        feeds = test_supabase_client.table("feeds").select("*").eq("category", category).execute()

        # If feeds exist in this category, verify they have is_active = true
        for feed in feeds.data:
            assert feed["is_active"] is True, f"Feed '{feed['name']}' should have is_active=true"
...
def test_property_16_seed_script_duplicate_handling(
    test_supabase_client: Client, feed_name, category
):
    """
    **Validates: Requirements 4.8**
    ...
    try:
        ...
        error_msg = str(e).lower()
        assert (
            "duplicate" in error_msg or "unique" in error_msg
        ), f"Expected duplicate key error, got: {e}"

    # Verify original feed still exists
    feed_check = test_supabase_client.table("feeds").select("*").eq("id", feed_id).execute()
...
def test_property_17_updated_timestamp_trigger(
    ...
    **Validates: Requirements 8.8**
    ...
    assert new_dt > initial_dt, "New updated_at should be later than initial updated_at"
