...
async def test_property_3_batch_processing_resilience(
    ...
    assert len(set(llm_processed_articles)) == num_articles, \
    ...
    assert mock_llm.evaluate_batch.call_count > 0, \
        "LLM evaluate_batch should be called"
...
async def test_property_3_all_items_fail_gracefully(num_articles):
    ...
    assert len(llm_processed) == num_articles, \
    ...
    assert len(db_attempted) == num_articles, \
    ...
    assert mock_llm.evaluate_batch.call_count > 0, \
        "LLM evaluate_batch should be called"
    assert mock_supabase.insert_articles.call_count > 0, \
        "Database insert_articles should be called"
