...
@given(
    status=st.sampled_from(
        ["unread", "READ", "archived", "Unread", "Read", "Archived", "UNREAD", "read", "ARCHIVED"]
    )
)
@settings(max_examples=5, suppress_health_check=[HealthCheck.function_scoped_fixture])
...
@given(
    url=st.text().filter(
        lambda s: not (s.strip().startswith("http://") or s.strip().startswith("https://"))
    )
)
@settings(max_examples=5, suppress_health_check=[HealthCheck.function_scoped_fixture])
...
@given(
    text=st.text(min_size=1, max_size=10000), max_length=st.integers(min_value=1, max_value=5000)
)
@settings(max_examples=5, suppress_health_check=[HealthCheck.function_scoped_fixture])
...
def test_handle_database_error_unique_constraint(field_name):
    ...
    **Validates: Requirements 13.3**
    ...
    service = create_service()
    error = Exception(
        f'duplicate key value violates unique constraint "users_discord_id_key" DETAIL: Key ({field_name})=(test) already exists.'
    )
...
def test_handle_database_error_foreign_key_constraint(table_name):
    ...
    **Validates: Requirements 13.4**
    ...
    service = create_service()
    error = Exception(
        f'insert or update on table "articles" violates foreign key constraint "articles_feed_id_fkey" DETAIL: Key (feed_id)=(123) is not present in table "{table_name}".'
    )
...
def test_handle_database_error_check_constraint(constraint_name):
    ...
    **Validates: Requirements 13.5**
    ...
    service = create_service()
    error = Exception(
        f'new row for relation "articles" violates check constraint "{constraint_name}"'
    )
...
async def test_execute_with_retry_transient_errors(error_keyword, max_retries):
    ...
    **Validates: Requirements 15.6**
    ...
    result = await service._execute_with_retry(
        failing_operation, max_retries=max_retries, base_delay=0.01
    )
    ...
    **Validates: Requirements 16.7**
    ...
    try:
        if validation_type == "status":
            ...
            if isinstance(invalid_value, str) and invalid_value.strip().title() not in [
                "Unread",
                "Read",
                "Archived",
            ]:
            ...
            if not _is_valid_uuid(str(invalid_value) if invalid_value is not None else ""):
                ...
                assert str(invalid_value) in error_msg or (
                    invalid_value is None and "" in error_msg
                )
            ...
            if not (
                url_str.strip().startswith("http://") or url_str.strip().startswith("https://")
            ):
                with pytest.raises(ValueError) as exc_info:
