...
def valid_tinkering_indices(draw):
    ...
    return draw(st.one_of(st.none(), st.integers(min_value=1, max_value=5)))
...
def test_property_1_article_schema_structure_validation(article_data):
    ...
    **Validates: Requirements 2.1, 2.2, 2.3, 2.4, 2.5, 2.6, 2.7, 2.8**
    ...
    assert article.published_at is None or isinstance(
    ...
    if article.embedding is not None:
        assert isinstance(
            article.embedding, list
        ), f"embedding must be Optional[List[float]], got {type(article.embedding)}"
        assert all(
            isinstance(x, float) for x in article.embedding
