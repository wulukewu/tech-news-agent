...
class TestOnboardingWorkflow:
    ...
    async def test_complete_onboarding_flow(self, onboarding_service, mock_supabase_client):
        ...
        mock_select_response.data = [{"user_id": str(user_id), "onboarding_started_at": None}]
        ...
        mock_select_response.data = [
            {
                "user_id": str(user_id),
                "onboarding_started_at": datetime.now(UTC).isoformat(),
            }
        ]
    ...
    async def test_reset_and_restart_onboarding(self, onboarding_service, mock_supabase_client):
        ...
        mock_select_response = Mock()
        mock_select_response.data = [
            {"user_id": str(user_id), "onboarding_completed": True, "onboarding_step": "complete"}
        ]
...
class TestEdgeCases:
    ...
    @pytest.mark.asyncio
    async def test_multiple_progress_updates_same_step(
        self, onboarding_service, mock_supabase_client
    ):
...
class TestDataConsistency:
    ...
    @pytest.mark.asyncio
    async def test_update_includes_updated_at_timestamp(
        self, onboarding_service, mock_supabase_client
    ):
