...
def mock_analytics_event_repo():
    """Create a mock AnalyticsEventRepository"""
...
@pytest.fixture
def analytics_service(mock_analytics_event_repo):
    """Create an AnalyticsService instance with mock repository"""
    return AnalyticsService(mock_analytics_event_repo)
...
class TestLogEvent:
    ...
    @pytest.mark.asyncio
    async def test_log_event_success(self, analytics_service, mock_analytics_event_repo):
        ...
        mock_analytics_event_repo.create.return_value = None

        # Execute
        await analytics_service.log_event(user_id, event_type, event_data)

        # Verify create was called
        mock_analytics_event_repo.create.assert_called_once()
        create_call_args = mock_analytics_event_repo.create.call_args[0][0]
        assert create_call_args["user_id"] == str(user_id)
        assert create_call_args["event_type"] == event_type
        assert create_call_args["event_data"] == event_data
    ...
    @pytest.mark.asyncio
    async def test_log_event_step_completed_with_time(
        self, analytics_service, mock_supabase_client
    ):
    ...
    @pytest.mark.asyncio
    async def test_log_event_onboarding_skipped_with_step(
        self, analytics_service, mock_supabase_client
    ):
        """Test logging onboarding_skipped event with step information"""
        user_id = uuid4()
        event_type = "onboarding_skipped"
        event_data = {"step": "recommendations", "reason": "user_clicked_skip"}
...
class TestGetOnboardingCompletionRate:
    ...
    async def test_completion_rate_database_error(self, analytics_service, mock_supabase_client):
        ...
        # Mock database error
        mock_table = Mock()
        mock_table.select.return_value.eq.return_value.gte.return_value.lte.return_value.execute.side_effect = Exception(
            "Database error"
        )
        mock_supabase_client.table.return_value = mock_table
...
class TestGetDropOffRates:
    ...
    async def test_drop_off_rates_with_data(self, analytics_service, mock_supabase_client):
        ...
        for i, uid in enumerate(started_users):
            if i < 90:
                step_events.append({"user_id": uid, "event_data": {"step": "welcome"}})
            if i < 75:
                step_events.append({"user_id": uid, "event_data": {"step": "recommendations"}})
            if i < 70:
                step_events.append({"user_id": uid, "event_data": {"step": "complete"}})
...
class TestEdgeCases:
    ...
    async def test_log_event_with_complex_event_data(self, analytics_service, mock_supabase_client):
        ...
        event_data = {
            ...
            "category": "Tech News",
            "metadata": {"source": "onboarding_modal", "position": 1, "recommended": True},
        }
    ...
    @pytest.mark.asyncio
    async def test_completion_rate_all_users_completed(
        self, analytics_service, mock_supabase_client
    ):
