...
def test_cleanup_mechanism_handles_cascading_deletes(test_supabase_client, cleanup_test_data):
    ...
    subscription_query = test_supabase_client.table('user_subscriptions').select('*').eq('id', subscription['id']).execute()
...
def test_cleanup_mechanism_with_complex_relationships(test_supabase_client, cleanup_test_data):
    ...
    # Verify all records exist
    assert len(test_supabase_client.table('users').select('*').eq('id', user['id']).execute().data) == 1
    assert len(test_supabase_client.table('feeds').select('*').eq('id', feed['id']).execute().data) == 1
    assert len(test_supabase_client.table('articles').select('*').eq('id', article['id']).execute().data) == 1
    assert len(test_supabase_client.table('user_subscriptions').select('*').eq('id', subscription['id']).execute().data) == 1
    assert len(test_supabase_client.table('reading_list').select('*').eq('id', reading_list_entry['id']).execute().data) == 1
...
def test_cleanup_mechanism_is_independent_across_tests(test_supabase_client, cleanup_test_data):
    ...
    query_result = test_supabase_client.table('users').select('*').eq('discord_id', test_discord_id).execute()
    assert len(query_result.data) == 1
