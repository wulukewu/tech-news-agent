...
pytestmark = pytest.mark.skip(reason="response format mismatch")
...
def test_discord_login_redirects_to_discord(client):
    ...
    # 模擬環境變數已設定
    with (
        patch.object(settings, "discord_client_id", "test_client_id"),
        patch.object(
            settings, "discord_redirect_uri", "http://localhost:8000/api/auth/discord/callback"
        ),
    ):
        # 發送請求
        ...
        assert (
            query_params["redirect_uri"][0] == "http://localhost:8000/api/auth/discord/callback"
        ), "redirect_uri 應正確"
...
def test_discord_login_missing_client_id(client):
    ...
    """
    with (
        patch.object(settings, "discord_client_id", None),
        ...
        assert "DISCORD_CLIENT_ID" in response.json()["detail"], "錯誤訊息應提及 DISCORD_CLIENT_ID"
