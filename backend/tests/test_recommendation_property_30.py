...
async def test_property_30_immediate_appearance_on_recommendation(
    ...
    try:
        ...
        assert feed_id not in feed_ids_before, \
        ...
        assert feed_id in feed_ids_after, \
        ...
        assert test_feed.is_recommended is True, \
            "Feed should have is_recommended=True"
...
async def test_property_30_immediate_removal_on_unrecommendation(
    test_supabase_client,
    priority
):
    """
    ...
    try:
        ...
        assert feed_id in feed_ids_before, \
        ...
        assert feed_id not in feed_ids_after, \
...
@settings(suppress_health_check=[HealthCheck.function_scoped_fixture])
@given(
    initial_priority=recommendation_priorities,
    updated_priority=recommendation_priorities
)
async def test_property_30_immediate_priority_update(
    ...
    try:
        ...
        recommended_feeds_before = await service.get_recommended_feeds()
        test_feed_before = next(
            (f for f in recommended_feeds_before if str(f.id) == feed_id),
            None
        )
        
        assert test_feed_before is not None, \
            "Feed should appear in recommended feeds"
        assert test_feed_before.recommendation_priority == initial_priority, \
        ...
        recommended_feeds_after = await service.get_recommended_feeds()
        test_feed_after = next(
            (f for f in recommended_feeds_after if str(f.id) == feed_id),
            None
        )
        
        assert test_feed_after is not None, \
            "Feed should still appear in recommended feeds"
        assert test_feed_after.recommendation_priority == updated_priority, \
            f"Feed should IMMEDIATELY have updated priority={updated_priority}"
        
        # Verify ordering is correct (descending by priority)
        test_feed_priorities = [f.recommendation_priority for f in recommended_feeds_after 
                                if str(f.id) == feed_id or f.recommendation_priority >= updated_priority - 10]
        assert test_feed_priorities == sorted(test_feed_priorities, reverse=True), \
...
async def test_property_30_multiple_feeds_real_time_updates(
    test_supabase_client,
    priorities
):
    """
    ...
    try:
        ...
        for feed_id in created_feed_ids:
            assert feed_id not in initial_ids, \
                "Non-recommended feeds should not appear initially"
        
        # Mark each feed as recommended and verify immediate appearance
        for i, (feed_id, priority) in enumerate(zip(created_feed_ids, priorities)):
            # Mark as recommended
            await service.update_recommendation_status(
                feed_id=feed_id,
                is_recommended=True,
            ...
            for prev_feed_id in created_feed_ids[:i+1]:
                assert prev_feed_id in current_ids, \
                    f"Previously recommended feed should still be present"
        ...
        assert len(final_test_feeds) == len(priorities), \
        ...
        assert final_priorities == expected_priorities, \
...
async def test_property_30_toggle_recommendation_status(
    test_supabase_client,
    priority
):
    """
    ...
    try:
        ...
        assert feed_id not in ids_2, "Feed should IMMEDIATELY disappear after marking as not recommended"
        
        # Toggle 3: False -> True
        await service.update_recommendation_status(feed_id, True, priority)
        feeds_3 = await service.get_recommended_feeds()
        ids_3 = [str(f.id) for f in feeds_3]
        assert feed_id in ids_3, "Feed should IMMEDIATELY reappear after marking as recommended again"
...
async def test_property_30_concurrent_queries_consistency(
    test_supabase_client,
    priority
):
    """
    ...
    try:
        ...
        for i in range(3):
            ...
            assert feed_id in ids, \
                f"Feed should appear in query {i+1} while is_recommended=True"
...
async def test_property_30_updated_at_timestamp_on_status_change(
    test_supabase_client,
    priority
):
    """
    ...
    try:
        ...
        db_response_1 = test_supabase_client.table('feeds') \
            .select('*') \
            .eq('id', feed_id) \
            .execute()
        ...
        db_response_2 = test_supabase_client.table('feeds') \
            .select('*') \
            .eq('id', feed_id) \
            .execute()
