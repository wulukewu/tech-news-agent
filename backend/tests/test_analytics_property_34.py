...
async def test_property_34_step_completion_includes_time(
    ...
    **Validates: Requirements 14.5**
    ...
    assert len(db_response.data) > 0, \
        "Expected step_completed event record"
...
@pytest.mark.asyncio
@pytest.mark.property_test
async def test_property_34_recommendations_step_time_tracking(
    test_supabase_client,
    test_user
):
    """
    ...
    **Validates: Requirements 14.5**
    ...
    assert event_record['event_data']['time_spent_seconds'] == 120, \
        "Expected time_spent_seconds=120 for recommendations step"


@pytest.mark.asyncio
@pytest.mark.property_test
async def test_property_34_complete_step_time_tracking(
    test_supabase_client,
    test_user
):
    """
    ...
    **Validates: Requirements 14.5**
    ...
    assert event_record['event_data']['time_spent_seconds'] == 30, \
...
async def test_property_34_multiple_step_completions_preserve_time(
    ...
    **Validates: Requirements 14.5**
    ...
    assert len(db_response.data) >= len(step_completions), \
    ...
    for i, (expected_step, expected_time) in enumerate(step_completions):
        ...
        assert 'event_data' in event_record, \
            f"Expected event_data in step completion event {i}"
...
async def test_property_34_time_range_boundaries(
    ...
    **Validates: Requirements 14.5**
    ...
    await service.log_event(user_id, 'step_completed', {
        'step': 'welcome',
    ...
    assert len(db_response.data) >= 2, \
        "Expected at least 2 step completion event records"
    
    # Verify minimum time
    min_time_event = db_response.data[0]
    assert min_time_event['event_data']['time_spent_seconds'] == 1, \
        "Expected minimum time_spent_seconds=1"
    
    # Verify maximum time
    max_time_event = db_response.data[1]
    assert max_time_event['event_data']['time_spent_seconds'] == 300, \
        "Expected maximum time_spent_seconds=300"


@pytest.mark.asyncio
@pytest.mark.property_test
async def test_property_34_step_time_field_required(
    test_supabase_client,
    test_user
):
    """
    ...
    **Validates: Requirements 14.5**
    ...
    assert event_record['event_data']['time_spent_seconds'] == 60, \
        "Expected time_spent_seconds value to be preserved"


@pytest.mark.asyncio
@pytest.mark.property_test
async def test_property_34_step_time_isolated_per_user(
    test_supabase_client,
    test_user
):
    """
    ...
    **Validates: Requirements 14.5**
    ...
    try:
        ...
        await service.log_event(user1_id, 'step_completed', {
            'step': 'welcome',
            'time_spent_seconds': 45
        })
        await service.log_event(user2_id, 'step_completed', {
            'step': 'welcome',
        ...
        assert len(user1_events.data) > 0, \
            "Expected step completion event for user1"
        assert user1_events.data[0]['event_data']['time_spent_seconds'] == 45, \
            "User1 should have time_spent_seconds=45"
        
        # Verify user2 has correct time
        assert len(user2_events.data) > 0, \
            "Expected step completion event for user2"
        assert user2_events.data[0]['event_data']['time_spent_seconds'] == 90, \
...
@pytest.mark.asyncio
@pytest.mark.property_test
async def test_property_34_step_time_timestamp_correlation(
    test_supabase_client,
    test_user
):
    """
    ...
    **Validates: Requirements 14.5**
    ...
    assert len(db_response.data) > 0, \
        "Expected step completion event record"
    
    event_record = db_response.data[0]
    
    # Verify time_spent_seconds is preserved
    assert event_record['event_data']['time_spent_seconds'] == 75, \
