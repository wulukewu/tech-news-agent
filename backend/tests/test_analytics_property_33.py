...
async def test_property_33_skip_event_includes_step(test_supabase_client, test_user, skip_step):
    """
    ...
    **Validates: Requirements 14.4**
    ...
    assert (
        "step" in event_record["event_data"]
    ), "Expected 'step' field in event_data for skip events"

    assert (
        event_record["event_data"]["step"] == skip_step
    ), f"Expected step '{skip_step}', got '{event_record['event_data']['step']}'"


@pytest.mark.asyncio
@pytest.mark.property_test
async def test_property_33_skip_at_welcome_step(test_supabase_client, test_user):
    """
    ...
    **Validates: Requirements 14.4**
    ...
    event_record = db_response.data[0]
    assert (
        event_record["event_data"]["step"] == "welcome"
    ), "Expected step='welcome' for skip at welcome step"


@pytest.mark.asyncio
@pytest.mark.property_test
async def test_property_33_skip_at_recommendations_step(test_supabase_client, test_user):
    """
...
@pytest.mark.asyncio
@pytest.mark.property_test
async def test_property_33_skip_at_complete_step(test_supabase_client, test_user):
    """
    ...
    **Validates: Requirements 14.4**
    ...
    event_record = db_response.data[0]
    assert (
        event_record["event_data"]["step"] == "complete"
...
@given(
    skip_events=st.lists(
        st.tuples(skip_steps, st.integers(min_value=1, max_value=300)),  # time_spent_seconds
        min_size=1,
        max_size=3,
    )
)
async def test_property_33_multiple_skip_events_preserve_steps(
    test_supabase_client, test_user, skip_events
):
    """
    ...
    **Validates: Requirements 14.4**
    ...
    for skip_step, time_spent in skip_events:
        event_data = {"step": skip_step, "time_spent_seconds": time_spent}
        await service.log_event(user_id, "onboarding_skipped", event_data)
    ...
    for i, (expected_step, expected_time) in enumerate(skip_events):
        ...
        assert "event_data" in event_record, f"Expected event_data in skip event {i}"

        assert "step" in event_record["event_data"], f"Expected step field in skip event {i}"
...
async def test_property_33_skip_event_with_additional_metadata(
    test_supabase_client, test_user, skip_step, additional_data
):
    """
    ...
    **Validates: Requirements 14.4**
    ...
    service = AnalyticsService(test_supabase_client)
    user_id = test_user["id"]
    event_data = {"step": skip_step, **additional_data}
    ...
    assert (
        stored_event_data["step"] == skip_step
    ), f"Expected step '{skip_step}', got '{stored_event_data['step']}'"

    # Verify additional metadata is preserved
    assert (
        stored_event_data["reason"] == additional_data["reason"]
    ), f"Expected reason '{additional_data['reason']}', got '{stored_event_data['reason']}'"

    assert (
        stored_event_data["time_spent_seconds"] == additional_data["time_spent_seconds"]
    ), f"Expected time_spent_seconds {additional_data['time_spent_seconds']}"


@pytest.mark.asyncio
@pytest.mark.property_test
async def test_property_33_skip_event_step_field_required(test_supabase_client, test_user):
    """
    ...
    **Validates: Requirements 14.4**
    ...
    assert event_record["event_data"]["step"] == "welcome", "Expected step value to be preserved"


@pytest.mark.asyncio
@pytest.mark.property_test
async def test_property_33_skip_events_isolated_per_user(test_supabase_client, test_user):
    """
    ...
    **Validates: Requirements 14.4**
    ...
    try:
        ...
        assert len(user1_events.data) > 0, "Expected skip event for user1"
        assert (
            user1_events.data[0]["event_data"]["step"] == "welcome"
        ), "User1 should have skipped at welcome step"

        # Verify user2 skipped at recommendations
        assert len(user2_events.data) > 0, "Expected skip event for user2"
        assert (
            user2_events.data[0]["event_data"]["step"] == "recommendations"
...
@pytest.mark.asyncio
@pytest.mark.property_test
async def test_property_33_skip_event_timestamp_accuracy(test_supabase_client, test_user):
    """
    ...
    **Validates: Requirements 14.4**
    ...
    assert len(db_response.data) > 0, "Expected skip event record"
