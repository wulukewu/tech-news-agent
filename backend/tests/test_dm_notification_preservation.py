...
class TestDMNotificationPreservation:
    ...
    **Property 2: Preservation** - For first-time users (no sent articles),
    ...
    async def test_preservation_subscription_filtering_preserved(
        self, test_supabase_client, test_user
    ):
        """
        ...
        **Validates: Requirement 3.1**
        ...
        feed1_url = f"https://test-feed-1-{datetime.now(UTC).timestamp()}.com/rss.xml"
        feed1 = (
        ...
        feed2_url = f"https://test-feed-2-{datetime.now(UTC).timestamp()}.com/rss.xml"
        feed2 = (
            test_supabase_client.table("feeds")
            .insert(
                {
        ...
        test_supabase_client.table("user_subscriptions").insert(
            {"user_id": test_user["id"], "feed_id": feed1["id"]}
        ).execute()
    ...
    async def test_preservation_property_first_time_user_behavior(
        self, test_supabase_client, test_user, test_feed, article_count, days_back
    ):
        """
        ...
        **Validates: Requirements 3.1, 3.2, 3.3**
        ...
        test_supabase_client.table("user_subscriptions").upsert(
            {"user_id": test_user["id"], "feed_id": test_feed["id"]}, on_conflict="user_id,feed_id"
        ).execute()
