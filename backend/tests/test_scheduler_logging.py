...
class TestSchedulerLogging:
    ...
    async def test_logs_task_start_time_at_info_level(self, caplog):
        ...
        start_logs = [
            ...
            if "Starting background_fetch_job" in record.message
    ...
    async def test_logs_count_of_new_articles_found_at_info_level(self, caplog):
        ...
        test_feeds = [make_test_feed()]
        test_articles = [
            make_test_article(f"Article {i}", f"https://example.com/article{i}")
            for i in range(5)
        ]
        ...
        article_logs = [
            record for record in caplog.records
            if ("Found 5 new articles" in record.message or "New articles found: 5" in record.message)
            and record.levelno == logging.INFO
        ]
    ...
    async def test_logs_count_of_articles_analyzed_by_llm_at_info_level(self, caplog):
        ...
        analysis_logs = [
            record for record in caplog.records
            if "Articles processed: 7" in record.message
            and record.levelno == logging.INFO
        ]
    ...
    async def test_logs_count_of_articles_inserted_into_database_at_info_level(self, caplog):
        ...
        assert len(db_logs) >= 1, "Should log count of articles inserted into database at INFO level"
