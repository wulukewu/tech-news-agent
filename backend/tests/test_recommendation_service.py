...
@pytest.fixture
def mock_feed_repo():
    """Create a mock FeedRepository"""
    return AsyncMock(spec=FeedRepository)


@pytest.fixture
def mock_user_subscription_repo():
    """Create a mock UserSubscriptionRepository"""
    return AsyncMock(spec=UserSubscriptionRepository)


@pytest.fixture
def recommendation_service(mock_feed_repo, mock_user_subscription_repo):
    """Create a RecommendationService instance with mock repositories"""
    return RecommendationService(mock_feed_repo, mock_user_subscription_repo)
...
class TestGetRecommendedFeeds:
    ...
    async def test_get_recommended_feeds_returns_sorted_by_priority(
        self, recommendation_service, mock_feed_repo, sample_feed_data
    ):
        """Test that recommended feeds are returned sorted by priority (highest first)"""
        # Setup mock response
        mock_feed_repo.list_recommended_feeds.return_value = sample_feed_data
    ...
    async def test_get_recommended_feeds_with_user_subscription_status(
        self, recommendation_service, mock_supabase_client, sample_feed_data
    ):
    ...
    async def test_get_recommended_feeds_returns_empty_list_when_none_found(
        self, recommendation_service, mock_supabase_client
    ):
    ...
    async def test_get_recommended_feeds_raises_error_on_database_failure(
        ...
        with pytest.raises((ServiceError, DatabaseError)) as exc_info:
...
class TestUpdateRecommendationStatus:
    ...
    async def test_update_recommendation_status_updates_feed(
        ...
        await recommendation_service.update_recommendation_status(
            feed_id=feed_id, is_recommended=is_recommended, priority=priority
        )
    ...
    async def test_update_recommendation_status_can_unrecommend_feed(
        ...
        # Execute
        await recommendation_service.update_recommendation_status(
            feed_id=feed_id, is_recommended=False, priority=0
        )
    ...
    async def test_update_recommendation_status_raises_error_for_negative_priority(
        self, recommendation_service, mock_feed_repo
    ):
        """Test that negative priority values are rejected"""
        feed_id = uuid4()

        # Execute and verify exception
        with pytest.raises((ServiceError, DatabaseError, ValueError)) as exc_info:
            await recommendation_service.update_recommendation_status(
                feed_id=feed_id, is_recommended=True, priority=-10
            )
    ...
    async def test_update_recommendation_status_raises_error_when_feed_not_found(
        ...
        mock_feed_repo.update.return_value = None

        # Execute and verify exception
        with pytest.raises((ServiceError, DatabaseError)) as exc_info:
            await recommendation_service.update_recommendation_status(
                feed_id=feed_id, is_recommended=True, priority=100
            )
    ...
    async def test_update_recommendation_status_raises_error_on_database_failure(
        ...
        mock_feed_repo.update.side_effect = DatabaseError("Database error")
...
class TestPrivateHelperMethods:
    ...
    async def test_get_user_subscribed_feed_ids_returns_set_of_uuids(
        ...
        mock_response = Mock()
        mock_response.data = [{"feed_id": str(feed_id_1)}, {"feed_id": str(feed_id_2)}]
