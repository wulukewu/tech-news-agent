...
@pytest.mark.asyncio
@given(completed=st.booleans(), skipped=st.booleans())
@settings(suppress_health_check=[HealthCheck.function_scoped_fixture])
async def test_property_24_should_show_onboarding_logic(
    test_supabase_client, test_user, completed, skipped
):
    """
    ...
    assert (
        status.should_show_onboarding == expected_should_show
    ...
    assert (
        status.onboarding_completed == completed
    ), f"Expected onboarding_completed={completed} but got {status.onboarding_completed}"

    assert (
        status.onboarding_skipped == skipped
    ), f"Expected onboarding_skipped={skipped} but got {status.onboarding_skipped}"


@pytest.mark.asyncio
async def test_property_24_new_user_should_show_onboarding(test_supabase_client, test_user):
    """
    ...
    test_supabase_client.table("user_preferences").delete().eq("user_id", str(user_id)).execute()

    # Act - Query status (will create default preferences)
    status = await service.get_onboarding_status(user_id)

    # Assert - New user should see onboarding
    assert status.should_show_onboarding is True, "New user should have should_show_onboarding=True"

    assert status.onboarding_completed is False, "New user should have onboarding_completed=False"

    assert status.onboarding_skipped is False, "New user should have onboarding_skipped=False"
...
async def test_property_24_completed_user_should_not_show_onboarding(
    ...
    assert (
        status.should_show_onboarding is False
    ), "User who completed onboarding should have should_show_onboarding=False"

    assert status.onboarding_completed is True, "User should have onboarding_completed=True"


@pytest.mark.asyncio
async def test_property_24_skipped_user_should_not_show_onboarding(test_supabase_client, test_user):
    """
    ...
    assert status.onboarding_skipped is True, "User should have onboarding_skipped=True"


@pytest.mark.asyncio
async def test_property_24_in_progress_user_should_show_onboarding(test_supabase_client, test_user):
    """
    ...
    assert status.onboarding_step == "recommendations", "User should be on recommendations step"


@pytest.mark.asyncio
@given(step=st.sampled_from(["welcome", "recommendations", "complete"]))
@settings(suppress_health_check=[HealthCheck.function_scoped_fixture])
...
@pytest.mark.asyncio
@given(initial_completed=st.booleans(), initial_skipped=st.booleans())
@settings(suppress_health_check=[HealthCheck.function_scoped_fixture])
async def test_property_24_multiple_queries_consistent_result(
    test_supabase_client, test_user, initial_completed, initial_skipped
):
    """
    ...
    test_supabase_client.table("user_preferences").update(
        {"onboarding_completed": initial_completed, "onboarding_skipped": initial_skipped}
    ).eq("user_id", str(user_id)).execute()
    ...
    assert (
        status1.should_show_onboarding
        == status2.should_show_onboarding
        == status3.should_show_onboarding
    ...
    assert (
        status1.should_show_onboarding == expected_should_show
    ), f"Expected should_show_onboarding={expected_should_show} but got {status1.should_show_onboarding}"

    # Verify state is unchanged
    assert (
        status1.onboarding_completed == status3.onboarding_completed
    ), "Queries should not modify onboarding_completed state"

    assert (
        status1.onboarding_skipped == status3.onboarding_skipped
...
async def test_property_24_state_transition_completed_updates_should_show(
    ...
    assert (
        status_before.should_show_onboarding is True
    ...
    assert (
        status_after.should_show_onboarding is False
    ), "After completion, should_show_onboarding should be False"

    assert (
        status_after.onboarding_completed is True
...
@pytest.mark.asyncio
async def test_property_24_reset_restores_should_show_onboarding(test_supabase_client, test_user):
    """
    ...
    assert (
        status_completed.should_show_onboarding is False
    ), "After completion, should_show_onboarding should be False"
    ...
    assert (
        status_reset.should_show_onboarding is True
    ), "After reset, should_show_onboarding should be True"

    assert (
        status_reset.onboarding_completed is False
    ), "After reset, onboarding_completed should be False"

    assert (
        status_reset.onboarding_skipped is False
...
@given(
    actions=st.lists(
        st.sampled_from(["progress", "complete", "skip", "reset"]), min_size=1, max_size=5
    )
)
@settings(suppress_health_check=[HealthCheck.function_scoped_fixture])
async def test_property_24_action_sequence_maintains_correct_should_show(
    test_supabase_client, test_user, actions
):
    """
