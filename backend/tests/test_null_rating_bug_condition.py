    ...
    **Validates: Requirements 1.5, 1.6, 2.5, 2.6**
        ...
        **Validates: Requirements 1.5, 1.6, 2.5, 2.6**
        ...
        try:
            ...
            assert request.rating is None, (
                "Schema accepted null but stored different value"
            )
    ...
    async def test_bug3_api_endpoint_rejects_null_rating(self):
        ...
        **Validates: Requirements 1.5, 1.6, 2.5, 2.6**
        ...
        mock_supabase.client.table.return_value.update.return_value.eq.return_value.eq.return_value.execute.return_value = mock_response
        ...
        with patch("app.api.reading_list.get_current_user", new=mock_get_current_user), \
            ...
            response = client.patch(
                f"/api/reading-list/{article_id}/rating",
                json={"rating": None}
            )
    ...
    async def test_bug3_counterexample_documentation(self):
        ...
        **Validates: Requirements 1.5, 1.6, 2.5, 2.6**
        ...
        with patch("app.api.reading_list.get_current_user", new=mock_get_current_user):
            response = client.patch(
                f"/api/reading-list/{uuid4()}/rating",
                json={"rating": None}
            )
