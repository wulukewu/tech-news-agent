...
@given(scenario=feed_change_scenario_strategy())
@pytest.mark.asyncio
async def test_property_17_dynamic_feed_configuration(scenario):
    ...
    for removed_feed in removed_feeds:
        assert (
            removed_feed.url not in second_execution_feeds
    ...
    for added_feed in added_feeds:
        assert (
            added_feed.url in second_execution_feeds
    ...
    for removed_feed in removed_feeds:
        assert (
            removed_feed.url in first_execution_feeds
    ...
    for added_feed in added_feeds:
        assert (
            added_feed.url not in first_execution_feeds
...
async def test_property_17_feed_deactivation(num_feeds, num_to_deactivate):
    ...
    for i in range(num_feeds):
        all_feeds.append(
            RSSSource(
                name=f"Feed {i}", url=f"https://feed-{uuid4().hex[:8]}.com/rss", category="AI"
            )
        )
    ...
    assert (
        len(first_execution_feeds) == num_feeds
    ...
    assert len(second_execution_feeds) == len(
        active_feeds
    ...
    for deactivated_feed in deactivated_feeds:
        assert (
            deactivated_feed.url not in second_execution_feeds
    ...
    for active_feed in active_feeds:
        assert (
            active_feed.url in second_execution_feeds
...
async def test_property_17_feed_activation(num_initial_feeds, num_new_feeds):
    ...
    assert (
        len(first_execution_feeds) == num_initial_feeds
    ...
    for initial_feed in initial_feeds:
        assert (
            initial_feed.url in first_execution_feeds
        ), f"Initial feed {initial_feed.url} should appear in first execution"
        assert (
            initial_feed.url in second_execution_feeds
