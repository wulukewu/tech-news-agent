...
def mock_feeds():
    ...
    return [
        RSSSource(name="Feed 1", url="https://example.com/feed1", category="AI"),
        RSSSource(name="Feed 2", url="https://example.com/feed2", category="DevOps"),
    ]
...
async def test_log_no_feed_changes(mock_services, mock_feeds, caplog):
    ...
    assert any(
        "No feed changes since last execution" in record.message for record in caplog.records
    )
...
async def test_first_execution_no_comparison(mock_services, mock_feeds, caplog):
    ...
    assert any(
        "First execution - no previous feed list to compare" in record.message
...
async def test_feed_changes_with_multiple_adds_and_removes(mock_services, caplog):
    ...
    assert any(
        "3 added" in record.message and "2 removed" in record.message for record in caplog.records
    )
