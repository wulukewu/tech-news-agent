...
class TestDMNotificationIntegration:
    ...
    @pytest.mark.asyncio
    async def test_full_dm_notification_flow(self, test_supabase_client, test_user, test_feed):
        """
        ...
        **Validates: Requirements 2.1, 2.2, 3.1, 3.2, 3.3**
        ...
        try:
            ...
            first_query = await supabase_service.get_user_articles(
                discord_id=test_user["discord_id"], days=7, limit=20
            )
            ...
            await supabase_service.record_sent_articles(
                discord_id=test_user["discord_id"], article_ids=article_ids
            )
            ...
            for article in initial_articles + new_articles:
                try:
                    test_supabase_client.table("articles").delete().eq(
                        "id", article["id"]
                    ).execute()
                except Exception:
                    pass
    ...
    @pytest.mark.asyncio
    async def test_multiple_users_simultaneous_notifications(self, test_supabase_client, test_feed):
        """
        ...
        **Validates: Requirements 2.1, 2.2**
        ...
        users = []
        for i in range(3):
            discord_id = f"test_multi_user_{i}_{datetime.now(UTC).timestamp()}"
            result = (
        ...
        for user in users:
            test_supabase_client.table("user_subscriptions").insert(
                {"user_id": user["id"], "feed_id": test_feed["id"]}
            ).execute()
        ...
        try:
            ...
            async def send_to_user(user):
                ...
                user_articles = await supabase_service.get_user_articles(
                    discord_id=user["discord_id"], days=7, limit=20
                )

                # Record sent articles
                article_ids = [str(article.id) for article in user_articles]
                await supabase_service.record_sent_articles(
                    discord_id=user["discord_id"], article_ids=article_ids
                )
    ...
    @pytest.mark.asyncio
    async def test_edge_case_empty_sent_articles(self, test_supabase_client, test_user, test_feed):
        """
        ...
        **Validates: Requirements 3.1, 3.2, 3.3**
        ...
        try:
            ...
            assert len(query_result) == 5, (
                f"User with no sent articles should get all 5 articles, " f"got {len(query_result)}"
            )
    ...
    @pytest.mark.asyncio
    async def test_edge_case_all_articles_sent(self, test_supabase_client, test_user, test_feed):
        """
        ...
        **Validates: Requirements 2.1, 2.2**
        ...
        try:
            ...
            assert len(query_result) == 0, (
                f"User with all articles sent should get 0 articles, " f"got {len(query_result)}"
            )
    ...
    @pytest.mark.asyncio
    async def test_edge_case_partial_overlap(self, test_supabase_client, test_user, test_feed):
        """
        ...
        **Validates: Requirements 2.1, 2.2**
        ...
        try:
            ...
            await supabase_service.record_sent_articles(
                discord_id=test_user["discord_id"], article_ids=sent_article_ids
            )
            ...
            assert (
                len(second_query_ids.intersection(sent_ids_set)) == 0
            ), "Returned articles should not include sent articles"
    ...
    @pytest.mark.asyncio
    async def test_dm_notification_service_integration(self, test_supabase_client, test_feed):
        """
        ...
        **Validates: Requirements 2.1, 2.2**
        ...
        result = (
            test_supabase_client.table("users").insert({"discord_id": numeric_discord_id}).execute()
        )
        ...
        try:
            ...
            supabase_service = SupabaseService()
            subsequent_query = await supabase_service.get_user_articles(
                discord_id=numeric_discord_id, days=7, limit=20
            )
