...
async def test_property_25_reset_after_completion_allows_reonboarding(
    ...
    assert status_recompleted.onboarding_completed is True, \
        "User should be able to complete onboarding again after reset"


@pytest.mark.asyncio
async def test_property_25_reset_after_skip_allows_reonboarding(
    test_supabase_client,
    test_user
):
    """
    ...
    assert status_progress.onboarding_step == 'recommendations', \
        "User should be able to progress through onboarding after reset"


@pytest.mark.asyncio
async def test_property_25_reset_clears_timestamps(
    test_supabase_client,
    test_user
):
    """
    ...
    assert db_record_before.get('onboarding_started_at') is not None, \
        "onboarding_started_at should be set before reset"
    assert db_record_before.get('onboarding_completed_at') is not None, \
    ...
    assert db_record_after.get('onboarding_started_at') is None, \
        "onboarding_started_at should be None after reset"
    assert db_record_after.get('onboarding_completed_at') is None, \
...
@settings(suppress_health_check=[HealthCheck.function_scoped_fixture])
async def test_property_25_reset_clears_any_step(
    test_supabase_client,
    test_user,
    step
):
...
@pytest.mark.asyncio
async def test_property_25_reset_idempotency(
    test_supabase_client,
    test_user
):
    """
    ...
    assert status_first_reset.onboarding_completed == status_second_reset.onboarding_completed, \
        "Multiple resets should produce consistent onboarding_completed state"
    
    assert status_first_reset.onboarding_skipped == status_second_reset.onboarding_skipped, \
        "Multiple resets should produce consistent onboarding_skipped state"
    
    assert status_first_reset.onboarding_step == status_second_reset.onboarding_step, \
        "Multiple resets should produce consistent onboarding_step state"
    
    assert status_first_reset.should_show_onboarding == status_second_reset.should_show_onboarding, \
        "Multiple resets should produce consistent should_show_onboarding state"


@pytest.mark.asyncio
async def test_property_25_reset_on_new_user_is_safe(
    test_supabase_client,
    test_user
):
    """
...
@given(
    actions_before_reset=st.lists(
        st.sampled_from(['progress', 'complete', 'skip']),
        min_size=1,
        max_size=3
    )
)
...
@pytest.mark.asyncio
async def test_property_25_reset_allows_full_onboarding_cycle(
    test_supabase_client,
    test_user
):
    """
...
@pytest.mark.asyncio
async def test_property_25_reset_updates_updated_at_timestamp(
    test_supabase_client,
    test_user
):
    """
    ...
    assert new_updated_at is not None, \
        "updated_at should be set after reset"
    ...
    assert initial_updated_at is not None, \
        "initial updated_at should exist"
...
async def test_property_25_reset_preserves_non_onboarding_preferences(
    ...
    assert db_record.get('tooltip_tour_completed') is True, \
        "tooltip_tour_completed should be preserved after onboarding reset"
    
    assert db_record.get('preferred_language') == 'en-US', \
        "preferred_language should be preserved after onboarding reset"


@pytest.mark.asyncio
@given(
    reset_count=st.integers(min_value=1, max_value=5)
)
@settings(suppress_health_check=[HealthCheck.function_scoped_fixture])
