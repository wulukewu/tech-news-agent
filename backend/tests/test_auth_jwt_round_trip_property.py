...
def test_jwt_round_trip_property(user_id, discord_id):
    ...
    **Validates: Requirements 3.1, 3.2, 3.5**
    ...
    assert (
        extracted_discord_id == discord_id
    ...
    assert (
        new_payload["discord_id"] == discord_id
    ...
    assert (
        original_payload["sub"] == new_payload["sub"]
    ), "Both tokens should have the same 'sub' claim"
    assert (
        original_payload["discord_id"] == new_payload["discord_id"]
    ...
    **Validates: Requirements 3.1, 3.2, 3.5**
    ...
    for i in range(iterations):
        ...
        assert payload["sub"] == str(
            user_id
        ), f"At iteration {i}, user_id should be preserved: expected {user_id!s}, got {payload['sub']}"
        assert (
...
@given(
    ...
    discord_id=st.one_of(
        ...
        # Mixed case strings
        st.text(
            min_size=1, max_size=50, alphabet="abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ"
        ),
    ),
)
def test_jwt_round_trip_edge_cases_property(user_id, discord_id):
    ...
    **Validates: Requirements 3.1, 3.2, 3.5**
    ...
    assert (
        original_payload["discord_id"] == discord_id
    ...
    assert len(new_payload["discord_id"]) == len(
        discord_id
