...
@given(
    ...
    error_stage=error_stage_strategy()
)
@pytest.mark.asyncio
async def test_property_11_scheduler_robustness_database_errors(
    error_message,
    error_stage,
    caplog
):
    ...
    database_error_logged = any(
        'database' in record.message.lower() or 
        'supabase' in record.message.lower() or
    ...
    completion_logged = any(
        'execution completed' in record.message.lower() and
        'scheduler will continue' in record.message.lower()
        for record in caplog.records
...
@pytest.mark.asyncio
async def test_property_11_scheduler_robustness_retry_exhaustion(
    error_message,
    caplog
):
    """
    ...
    assert len(get_feeds_attempts) == 3, \
    ...
    retry_failure_logged = any(
        'retry' in record.message.lower() or
...
async def test_property_11_scheduler_robustness_partial_retry_success(
    ...
    with patch('app.tasks.scheduler.SupabaseService') as mock_supabase_class, \
        ...
        mock_supabase.insert_articles.return_value = BatchResult(
            inserted_count=len(test_articles),
            updated_count=0,
            failed_count=0,
            failed_articles=[]
        )
    ...
    assert len(get_feeds_attempts) == success_on_attempt, \
    ...
    assert mock_rss.fetch_new_articles.call_count > 0, \
        "RSS service should be called after successful retry"
    assert mock_llm.evaluate_batch.call_count > 0, \
        "LLM service should be called after successful retry"
    assert mock_supabase.insert_articles.call_count > 0, \
...
@pytest.mark.asyncio
async def test_property_11_scheduler_robustness_unexpected_exceptions(
    exception_type,
    caplog
):
    """
    ...
    exception_in_logs = any(
        exception_message in record.message or
        type(exception_type).__name__ in record.message
        for record in caplog.records
        if record.levelname == 'CRITICAL'
    )
