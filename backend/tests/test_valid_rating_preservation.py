    ...
    **Validates: Requirements 3.6, 3.7**
    ...
    def test_preservation_schema_rejects_out_of_range_ratings(self, rating):
        ...
        **Validates: Requirements 3.6, 3.7**
        ...
        assert "rating" in error_msg or "value" in error_msg, (
    ...
    async def test_preservation_schema_validation_with_invalid_ratings(self):
        ...
        **Validates: Requirements 3.6, 3.7**
        ...
        for rating in invalid_ratings:
            ...
            # Verify the error message is appropriate
            error_msg = str(exc_info.value).lower()
            assert "rating" in error_msg or "value" in error_msg or "greater" in error_msg or "less" in error_msg, (
        ...
        **Validates: Requirements 3.6, 3.7**
        ...
        for rating in [0, -1, 6, 10, 100]:
            try:
                UpdateRatingRequest(rating=rating)
                pytest.fail(
                    f"Baseline behavior violated: Schema should reject rating={rating}"
                )
            except ValidationError:
        ...
        assert len(preserved_behaviors) >= 12, (
