...
def url_strategy():
    ...
    paths = st.lists(st.text(alphabet=path_chars, min_size=1, max_size=20), min_size=0, max_size=3)
...
def datetime_strategy():
    ...
    return st.datetimes(min_value=min_date, max_value=now, timezones=st.just(UTC))
...
@given(num_articles=st.integers(min_value=1, max_value=10), data=st.data())
@pytest.mark.asyncio
async def test_property_18_serialization_roundtrip(
    test_supabase_client, test_feed, cleanup_test_data, num_articles, data
):
    """
    Property 18: Serialization Round-Trip
    ...
    assert (
        result.inserted_count == num_articles
    ...
    for original_article in original_articles:
        ...
        if original_article.published_at is not None:
            ...
            if original_timestamp.tzinfo is None:
                original_timestamp = original_timestamp.replace(tzinfo=UTC)
            if db_timestamp.tzinfo is None:
                db_timestamp = db_timestamp.replace(tzinfo=UTC)
            ...
            assert (
                db_article["published_at"] is None or db_article["published_at"] is not None
        ...
        if original_article.tinkering_index is None:
            assert db_article["tinkering_index"] is None, "NULL tinkering_index should be preserved"
        else:
            assert (
                db_article["tinkering_index"] == original_article.tinkering_index
        ...
        if original_article.ai_summary is None:
            assert db_article["ai_summary"] is None, "NULL ai_summary should be preserved"
        else:
            assert (
                db_article["ai_summary"] == original_article.ai_summary
...
async def test_property_18_special_characters_preservation(
    test_supabase_client, test_feed, cleanup_test_data, title_with_special_chars, url_path
):
    """
    ...
    db_result = (
        test_supabase_client.table("articles").select("*").eq("url", str(article.url)).execute()
    )
...
async def test_property_18_timezone_preservation(
    test_supabase_client, test_feed, cleanup_test_data, timestamp
):
    """
    ...
    assert db_timestamp.tzinfo == UTC, f"Timezone should be UTC, got {db_timestamp.tzinfo}"
