...
async def test_active_feeds_filtering(test_supabase_client, cleanup_test_data):
    ...
    **Validates: Requirements 4.2, 4.3**
    ...
    active_feed = test_supabase_client.table('feeds').insert({
        'name': 'Active Feed',
    ...
    inactive_feed = test_supabase_client.table('feeds').insert({
        'name': 'Inactive Feed',
...
@pytest.mark.asyncio
async def test_article_upsert_idempotency(test_supabase_client, test_feed, cleanup_test_data, article_data):
    """
    **Validates: Requirements 5.2, 5.3, 5.4**
    ...
    existing = test_supabase_client.table('articles').select('id').eq('url', article_data['url']).execute()
    ...
    # Verify only one record in database
    articles = test_supabase_client.table('articles').select('*').eq('url', article_data['url']).execute()
...
@pytest.mark.asyncio
async def test_batch_operation_statistics_accuracy(test_supabase_client, test_feed, cleanup_test_data):
    """
    **Validates: Requirements 5.7, 15.4**
    ...
    for article in articles[:2]:
        articles_in_db = test_supabase_client.table('articles').select('id').eq('url', article['url']).execute()
...
@given(discord_id=discord_ids())
@pytest.mark.asyncio
async def test_reading_list_upsert_idempotency(test_supabase_client, test_article, cleanup_test_data, discord_id):
    """
    **Validates: Requirements 6.3, 6.4**
    ...
    entries = test_supabase_client.table('reading_list')\
        .select('*')\
...
@given(discord_id=discord_ids())
@pytest.mark.asyncio
async def test_reading_list_initial_status(test_supabase_client, test_article, cleanup_test_data, discord_id):
    """
    **Validates: Requirements 6.5**
    ...
    entry = test_supabase_client.table('reading_list')\
        .select('*')\
        .eq('user_id', str(user_uuid))\
        .eq('article_id', str(article_id))\
        .execute()
...
@pytest.mark.asyncio
async def test_status_update_persistence(test_supabase_client, test_article, cleanup_test_data, discord_id, status):
    """
    **Validates: Requirements 7.5, 7.6**
    ...
    initial_entry = test_supabase_client.table('reading_list')\
        .select('updated_at')\
        .eq('user_id', str(user_uuid))\
        .eq('article_id', str(article_id))\
        .execute()
    ...
    updated_entry = test_supabase_client.table('reading_list')\
        .select('*')\
        .eq('user_id', str(user_uuid))\
        .eq('article_id', str(article_id))\
        .execute()
...
@pytest.mark.asyncio
async def test_rating_update_persistence(test_supabase_client, test_article, cleanup_test_data, discord_id, rating):
    """
...
async def test_reading_list_status_filtering(test_supabase_client, test_user, cleanup_test_data):
    ...
    **Validates: Requirements 9.3**
    ...
    for status in statuses_to_test:
        article = test_supabase_client.table('articles').insert({
            'feed_id': feed['id'],
        ...
        entry = test_supabase_client.table('reading_list').insert({
...
@given(discord_id=discord_ids())
@pytest.mark.asyncio
async def test_subscription_idempotency(test_supabase_client, test_feed, cleanup_test_data, discord_id):
    """
    **Validates: Requirements 11.3, 11.4**
    ...
    subscriptions = test_supabase_client.table('user_subscriptions')\
        .select('*')\
        .eq('user_id', str(user_uuid))\
...
@given(discord_id=discord_ids())
@pytest.mark.asyncio
async def test_unsubscription_idempotency(test_supabase_client, test_feed, cleanup_test_data, discord_id):
    """
...
async def test_user_subscriptions_ordering(test_supabase_client, cleanup_test_data):
    ...
    **Validates: Requirements 12.4**
    ...
    for sub in subscriptions:
        subs_in_db = test_supabase_client.table('user_subscriptions')\
            .select('id')\
            .eq('user_id', str(user_uuid))\
