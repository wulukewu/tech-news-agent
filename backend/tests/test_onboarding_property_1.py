...
async def test_property_1_update_progress_persistence(
    ...
    assert isinstance(status, OnboardingStatus), \
        f"Expected OnboardingStatus but got {type(status)}"
    ...
    assert db_record['onboarding_step'] == step, \
...
async def test_property_1_multiple_updates_persistence(
    ...
    assert status.onboarding_step == final_step, \
    ...
    db_record = db_response.data[0]
    assert db_record['onboarding_step'] == final_step, \
        f"Database should have final step '{final_step}' but has '{db_record['onboarding_step']}'"


@pytest.mark.asyncio
async def test_property_1_mark_completed_persistence(
    test_supabase_client,
    test_user
):
    """
    ...
    assert status.onboarding_completed is True, \
        "Expected onboarding_completed to be True"
    ...
    assert db_record['onboarding_step'] == 'complete', \
        "Database record should have step='complete'"


@pytest.mark.asyncio
async def test_property_1_mark_skipped_persistence(
    test_supabase_client,
    test_user
):
    """
...
async def test_property_1_any_action_persistence(
    test_supabase_client,
    test_user,
    action,
    step
    ...
    if expected_step is not None:
        assert status.onboarding_step == expected_step, \
    ...
    assert len(db_response.data) == 1, \
        "Expected exactly one user_preferences record in database"


@pytest.mark.asyncio
@given(
    steps=st.lists(onboarding_steps, min_size=1, max_size=5)
)
async def test_property_1_sequence_persistence(
    test_supabase_client,
    test_user,
    steps
):
...
@pytest.mark.asyncio
async def test_property_1_started_at_timestamp_persistence(
    test_supabase_client,
    test_user
):
    """
    ...
    assert first_started_at is not None, \
        "onboarding_started_at should be set after first update"
...
@pytest.mark.asyncio
async def test_property_1_updated_at_timestamp_persistence(
    test_supabase_client,
    test_user
):
    """
    ...
    assert first_updated_at is not None, \
        "updated_at should be set after first action"
    ...
    assert second_updated_at is not None, \
        "updated_at should be set after second action"
...
@pytest.mark.asyncio
async def test_property_1_persistence_survives_service_recreation(
    test_supabase_client,
    test_user
):
    """
    ...
    db_record = db_response.data[0]
    assert db_record['onboarding_step'] == 'recommendations', \
