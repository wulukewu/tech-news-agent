...
def article_data_strategy(feed_id):
    ...
    return st.builds(
        lambda title, url: {"title": title, "url": url, "feed_id": str(feed_id)},
...
async def test_property_6_article_insertion_idempotence(
    ...
    for i in range(num_unique_articles):
        ...
        article = {"title": f"Test Article {i}", "url": unique_url, "feed_id": str(feed_id)}
    ...
    for insertion_round in range(num_duplicate_insertions):
        ...
        articles_to_insert = [
            {**article, "title": f"{article['title']} - Round {insertion_round}"}
    ...
    for i, result in enumerate(all_results[1:], start=2):
        ...
        assert (
            result.updated_count == num_unique_articles
...
async def test_property_6_mixed_insert_and_update(
    test_supabase_client, test_feed, cleanup_test_data, num_new_articles, num_existing_articles
):
    """
    ...
    for i in range(num_existing_articles):
        ...
        article = {"title": f"Existing Article {i}", "url": url, "feed_id": str(feed_id)}
    ...
    for i, article in enumerate(existing_articles):
        updated_article = {**article, "title": f"Updated Article {i}"}
    ...
    for i in range(num_new_articles):
        ...
        article = {"title": f"New Article {i}", "url": url, "feed_id": str(feed_id)}
...
@given(num_insertions=st.integers(min_value=2, max_value=10))
@pytest.mark.asyncio
async def test_property_6_rapid_duplicate_insertions(
    test_supabase_client, test_feed, cleanup_test_data, num_insertions
):
    """
    ...
    article = {"title": "Original Title", "url": url, "feed_id": str(feed_id)}

    # Act: Insert the same article multiple times rapidly
    results = []
    for i in range(num_insertions):
        # Modify title to simulate updates
        article_to_insert = {**article, "title": f"Title Version {i}"}
...
async def test_property_6_upsert_with_optional_fields(
    ...
    initial_article = {"title": "Initial Article", "url": url, "feed_id": str(feed_id)}
    ...
    for i in range(num_updates):
        updated_article = {"title": f"Updated Article {i}", "url": url, "feed_id": str(feed_id)}
    ...
    if has_ai_summary:
        ...
        assert (
            article_data["ai_summary"] == expected_summary
