...
def test_cookie_security_attributes_property(user_id, discord_id):
    ...
    **Validates: Requirements 4.1, 4.2, 4.3, 4.4, 4.5, 4.6**
    ...
    assert cookie_name_value.startswith(
        "access_token="
    ...
    assert "httponly" in cookie_attributes, "Cookie should have the HttpOnly flag set"
    assert cookie_attributes["httponly"] is True, "Cookie HttpOnly flag should be True"
    ...
    assert "samesite" in cookie_attributes, "Cookie should have the SameSite attribute set"
    assert (
        cookie_attributes["samesite"] == "lax"
...
def test_cookie_security_attributes_production_mode_property(user_id, discord_id):
    ...
    **Validates: Requirements 4.1, 4.2, 4.3, 4.4, 4.5, 4.6**
    ...
    try:
        ...
        assert "path" in cookie_attributes, "Cookie should have Path attribute in production mode"
        assert cookie_attributes["path"] == "/", "Cookie Path should be '/' in production mode"
...
def test_cookie_security_attributes_development_mode_property(user_id, discord_id):
    ...
    **Validates: Requirements 4.1, 4.2, 4.3, 4.4, 4.5, 4.6**
    ...
    try:
        ...
        assert "path" in cookie_attributes, "Cookie should have Path attribute in development mode"
        assert cookie_attributes["path"] == "/", "Cookie Path should be '/' in development mode"
...
@given(
    ...
    extra_claim=st.one_of(
        st.none(),
        st.text(
            min_size=0, max_size=200, alphabet=st.characters(min_codepoint=32, max_codepoint=126)
        ),
    ),
)
def test_cookie_security_attributes_edge_cases_property(user_id, discord_id, extra_claim):
    ...
    **Validates: Requirements 4.1, 4.2, 4.3, 4.4, 4.5, 4.6**
    ...
    assert "access_token=" in set_cookie_header, "Set-Cookie header should contain 'access_token='"
    ...
    assert (
        "SameSite=lax" in set_cookie_header or "samesite=lax" in set_cookie_header.lower()
    ), "Set-Cookie header should contain SameSite=lax"

    assert (
        "Max-Age=" in set_cookie_header or "max-age=" in set_cookie_header.lower()
    ), "Set-Cookie header should contain Max-Age attribute"

    assert (
        "Path=/" in set_cookie_header or "path=/" in set_cookie_header.lower()
    ...
    assert cookie_name_value.startswith("access_token="), "Cookie should start with 'access_token='"

    # Verify the cookie value doesn't contain semicolons or other delimiters
    # that would break the cookie format
    cookie_value = cookie_name_value.split("=", 1)[1]
    assert ";" not in cookie_value, "Cookie value should not contain semicolons"
    assert "\n" not in cookie_value, "Cookie value should not contain newlines"
    assert "\r" not in cookie_value, "Cookie value should not contain carriage returns"
