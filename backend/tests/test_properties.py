...
def test_property_2_feed_deletion_cascades(
    ...
    **Validates: Requirements 3.10**
    ...
    article_check = test_supabase_client.table('articles')\
        .select('*')\
        .eq('id', article['id'])\
        .execute()
...
def test_property_4_discord_id_uniqueness(
    ...
    **Validates: Requirements 6.1**
    ...
    # Insert first user
    user1_result = test_supabase_client.table('users').insert({
        'discord_id': unique_discord_id
    }).execute()
...
def test_property_5_subscription_uniqueness(
    ...
    **Validates: Requirements 6.4**
    ...
    subscription1_result = test_supabase_client.table('user_subscriptions').insert({
        'user_id': user['id'],
        'feed_id': feed['id']
    }).execute()
...
def test_property_6_reading_list_entry_uniqueness(
    ...
    **Validates: Requirements 6.5**
    ...
    entry1_result = test_supabase_client.table('reading_list').insert({
        'user_id': user['id'],
        'article_id': article['id'],
        'status': 'Unread'
    }).execute()
...
def test_property_7_feed_url_uniqueness(
    ...
    **Validates: Requirements 7.3**
    ...
    # Insert first feed
    feed1_result = test_supabase_client.table('feeds').insert({
        'name': 'Test Feed 1',
        'url': unique_feed_url,
        'category': 'Test'
    }).execute()
...
def test_property_8_article_url_uniqueness(
    ...
    **Validates: Requirements 7.4**
    ...
    article1_result = test_supabase_client.table('articles').insert({
        'feed_id': feed['id'],
...
def test_property_9_shared_feed_references(
    ...
    **Validates: Requirements 7.5**
    ...
    # Create two users
    user1_result = test_supabase_client.table('users').insert({
        'discord_id': unique_discord_id1
    }).execute()
    user1 = user1_result.data[0]
    cleanup_test_data['users'].append(user1['id'])
    
    user2_result = test_supabase_client.table('users').insert({
        'discord_id': unique_discord_id2
    }).execute()
    ...
    subscription2_result = test_supabase_client.table('user_subscriptions').insert({
        'user_id': user2['id'],
        'feed_id': feed['id']
    }).execute()
    ...
    assert subscription1['feed_id'] == subscription2['feed_id'], \
        "Both subscriptions should reference the same feed_id"
    assert subscription1['feed_id'] == feed['id'], \
        "Subscriptions should reference the created feed"
    
    # Verify only one feed exists with this URL
    feeds_check = test_supabase_client.table('feeds')\
        .select('*')\
        .eq('url', unique_feed_url)\
        .execute()
...
def test_property_10_required_field_validation(
    ...
    **Validates: Requirements 9.3, 9.4, 9.5, 9.6, 9.7, 9.8**
    ...
    if field_choice == 'discord_id':
        ...
        error_message = str(exc_info.value).lower()
        assert 'null' in error_message or 'required' in error_message or 'violates' in error_message, \
...
def test_property_12_reading_list_status_validation(
    ...
    **Validates: Requirements 9.1**
    ...
    error_message = str(exc_info.value).lower()
    assert 'check' in error_message or 'constraint' in error_message or 'violates' in error_message, \
...
def test_property_15_seed_script_active_flag(test_supabase_client, cleanup_test_data):
    ...
    **Validates: Requirements 4.7**
    ...
    feed_result2 = test_supabase_client.table('feeds').insert({
        'name': 'Default Feed',
...
def test_property_16_seed_script_duplicate_handling(
    ...
    **Validates: Requirements 4.8**
    ...
    try:
        ...
        url_check = test_supabase_client.table('feeds')\
            .select('*')\
            .eq('url', unique_feed_url)\
            .execute()
...
def test_property_17_updated_timestamp_trigger(
    ...
    **Validates: Requirements 8.8**
    ...
    entry_result = test_supabase_client.table('reading_list').insert({
        'user_id': user['id'],
        'article_id': article['id'],
    ...
    assert updated_entry['status'] == updated_status, \
