...
@pytest.mark.asyncio
async def test_property_2_completion_sets_flag(
...
@pytest.mark.asyncio
async def test_property_2_completion_records_timestamp(
    test_supabase_client,
    test_user
):
    """
    ...
    assert before_completion <= completed_at_dt <= after_completion, \
...
async def test_property_2_completion_prevents_multiple_updates(
    ...
    assert status_final.onboarding_completed is True, \
...
@pytest.mark.asyncio
async def test_property_2_completion_sets_step_to_complete(
    test_supabase_client,
    test_user
):
    """
    ...
    assert status_after.onboarding_step == 'complete', \
        f"Expected step to be 'complete' but got '{status_after.onboarding_step}'"


@pytest.mark.asyncio
@given(
    initial_step=onboarding_steps
)
async def test_property_2_completion_from_any_step(
    test_supabase_client,
    test_user,
    initial_step
):
    ...
    assert status.onboarding_step == 'complete', \
        f"Expected step to be 'complete' after completing from '{initial_step}'"


@pytest.mark.asyncio
async def test_property_2_completion_idempotent(
    test_supabase_client,
    test_user
):
    """
    ...
    db_response_1 = test_supabase_client.table('user_preferences') \
        .select('*') \
        .eq('user_id', str(user_id)) \
        .execute()
    ...
    db_response_2 = test_supabase_client.table('user_preferences') \
        .select('*') \
        .eq('user_id', str(user_id)) \
        .execute()
    ...
    assert status.onboarding_completed is True, \
        "Expected onboarding_completed to remain True"
    
    assert status.onboarding_step == 'complete', \
        "Expected step to remain 'complete'"
...
@pytest.mark.asyncio
async def test_property_2_completion_state_persists_across_queries(
    test_supabase_client,
    test_user
):
    """
    ...
    for i in range(5):
        ...
        assert status.onboarding_step == 'complete', \
            f"Query {i+1}: Expected step to be 'complete'"
...
@pytest.mark.asyncio
async def test_property_2_completion_does_not_affect_skip_flag(
    test_supabase_client,
    test_user
):
    """
...
@pytest.mark.asyncio
async def test_property_2_completion_after_skip(
    test_supabase_client,
    test_user
):
    """
    ...
    assert status_after_complete.onboarding_step == 'complete', \
...
@pytest.mark.asyncio
async def test_property_2_completion_database_consistency(
    test_supabase_client,
    test_user
):
    """
    ...
    assert api_status.onboarding_completed == db_record['onboarding_completed'], \
        "API and database onboarding_completed should match"
    
    assert api_status.onboarding_step == db_record['onboarding_step'], \
        "API and database onboarding_step should match"
    
    assert db_record['onboarding_completed'] is True, \
        "Database should have onboarding_completed=True"
    
    assert db_record['onboarding_step'] == 'complete', \
        "Database should have onboarding_step='complete'"
    
    assert db_record['onboarding_completed_at'] is not None, \
