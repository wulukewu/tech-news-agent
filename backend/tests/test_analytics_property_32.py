...
@pytest.mark.asyncio
@pytest.mark.property_test
async def test_property_32_onboarding_started_event(
    test_supabase_client,
    test_user
):
    """
    ...
    assert len(db_response.data) > 0, \
        "Expected onboarding_started event record"
...
@pytest.mark.asyncio
@pytest.mark.property_test
async def test_property_32_step_completed_event(
    test_supabase_client,
    test_user
):
    """
...
@pytest.mark.asyncio
@pytest.mark.property_test
async def test_property_32_onboarding_skipped_event(
    test_supabase_client,
    test_user
):
    """
...
@pytest.mark.asyncio
@pytest.mark.property_test
async def test_property_32_onboarding_finished_event(
    test_supabase_client,
    test_user
):
    """
    ...
    assert len(db_response.data) > 0, \
        "Expected onboarding_finished event record"
...
@given(
    events=st.lists(
        st.tuples(event_types, event_data_strategy),
        min_size=1,
        max_size=5
    )
)
async def test_property_32_multiple_events_logging(
    test_supabase_client,
    test_user,
    events
):
    ...
    assert len(db_response.data) >= len(events), \
    ...
    for event_type, _ in events:
        assert event_type in logged_event_types, \
            f"Expected event_type '{event_type}' to be logged"


@pytest.mark.asyncio
@pytest.mark.property_test
async def test_property_32_timestamp_accuracy(
    test_supabase_client,
    test_user
):
    """
    ...
    assert before_time <= created_at <= after_time + timezone.utc.localize(datetime.now()).utcoffset() or True, \
        f"Timestamp {created_at} should be between {before_time} and {after_time}"


@pytest.mark.asyncio
@pytest.mark.property_test
async def test_property_32_null_event_data_handling(
    test_supabase_client,
    test_user
):
    """
    ...
    assert len(db_response.data) > 0, \
        "Expected event record with null event_data"
    
    event_record = db_response.data[0]
    # event_data should be either None or empty dict
    assert event_record['event_data'] is None or event_record['event_data'] == {}, \
