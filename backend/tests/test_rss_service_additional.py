...
class TestRSSServiceDefaults:
    ...
    async def test_default_time_window_filters_correctly(self):
        ...
        recent_struct = (datetime.now(timezone.utc) - timedelta(days=5)).timetuple()[:6] + (0, 0, 0)
        old_struct = (datetime.now(timezone.utc) - timedelta(days=8)).timetuple()[:6] + (0, 0, 0)
...
class TestFetchNewArticlesErrorHandling:
    ...
    async def test_fetch_all_feeds_failure_propagates(self):
        ...
        with patch.object(service, 'fetch_all_feeds', AsyncMock(side_effect=Exception("Network error"))):
...
class TestTimeWindowConfiguration:
    ...
    async def test_custom_time_window_1_day(self):
        ...
        hours_12_struct = (datetime.now(timezone.utc) - timedelta(hours=12)).timetuple()[:6] + (0, 0, 0)
        days_2_struct = (datetime.now(timezone.utc) - timedelta(days=2)).timetuple()[:6] + (0, 0, 0)

        mock_feed = MagicMock()
        mock_feed.entries = [
            {"title": "Within1Day", "link": "https://example.com/1", "summary": "content",
             "published": "recent", "published_parsed": hours_12_struct},
            {"title": "Beyond1Day", "link": "https://example.com/2", "summary": "content",
             "published": "old", "published_parsed": days_2_struct},
        ]
    ...
    async def test_custom_time_window_30_days(self):
        ...
        # Create articles at different ages
        days_20_struct = (datetime.now(timezone.utc) - timedelta(days=20)).timetuple()[:6] + (0, 0, 0)
        days_35_struct = (datetime.now(timezone.utc) - timedelta(days=35)).timetuple()[:6] + (0, 0, 0)

        mock_feed = MagicMock()
        mock_feed.entries = [
            {"title": "Within30Days", "link": "https://example.com/1", "summary": "content",
             "published": "recent", "published_parsed": days_20_struct},
            {"title": "Beyond30Days", "link": "https://example.com/2", "summary": "content",
             "published": "old", "published_parsed": days_35_struct},
        ]
...
class TestPublishedAtHandling:
    ...
    async def test_article_with_missing_published_at_is_included(self):
        ...
        assert (datetime.now(timezone.utc) - result[0].published_at).total_seconds() < 5
