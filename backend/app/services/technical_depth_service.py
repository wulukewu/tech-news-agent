...
class TechnicalDepthService:
    ...
    def estimate_article_depth(content: str, title: str = "") -> str:
        ...
        # For mixed English/Chinese text, count Chinese characters as separate words to get correct density
        total_words = len(text.split()) + sum(1 for char in text if "\u4e00" <= char <= "\u9fff")
        if total_words == 0:
