...
class SubscriptionToggleRequest(BaseModel):
    ...
    model_config = ConfigDict(
        json_schema_extra={"example": {"feed_id": "123e4567-e89b-12d3-a456-426614174000"}}
    )
...
class SubscriptionToggleResponse(BaseModel):
    ...
    model_config = ConfigDict(
        json_schema_extra={
            "example": {"feed_id": "123e4567-e89b-12d3-a456-426614174000", "is_subscribed": True}
        }
    )
...
class BatchSubscribeResponse(BaseModel):
    ...
    errors: list[str] = Field(
        default_factory=list, description="List of error messages for failed subscriptions"
    )
