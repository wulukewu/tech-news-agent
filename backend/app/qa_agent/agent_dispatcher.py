...
class QAAgentDispatcher:
    ...
    async def dispatch(
        ...
        system_prompt = (
            ...
            "1. `chat`：使用者在進行閒聊、打招呼（如「你好」）、詢問一般技術概念（如「解釋一下什麼是 HMR？」）或詢問系統怎麼用。你需要在 `reply_content` 中用繁體中文熱情回答，回答內容要貼合使用者的技術背景和偏好設定。\n"
            ...
            "3. `search`：使用者想要尋找「特定主題的文章」或尋找「最近有哪些文章」（例如「幫我找找關於 AI 的新技術文章」）。你需要提供對應的搜尋關鍵字並寫在 `search_query` 中。此時 `reply_content` 應設為 null。\n\n"
            "【輸出規範】\n"
