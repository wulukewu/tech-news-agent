...
async def log_analytics_event(
    request: LogAnalyticsEventRequest, current_user: dict[str, Any] = Depends(get_current_user)
):
    """
    ...
    try:
        ...
        return success_response({"message": "事件已記錄"})

    except AnalyticsServiceError as e:
        logger.error(
            f"Failed to log analytics event for user {current_user['user_id']}: {e!s}",
            exc_info=True,
        )
        raise HTTPException(status_code=500, detail="無法記錄事件，請稍後再試")
    except Exception as e:
        logger.error(
            f"Unexpected error logging analytics event for user {current_user['user_id']}: {e!s}",
            exc_info=True,
        )
...
@router.get(
    ...
    response_model=SuccessResponse[OnboardingCompletionRateResponse],
)
async def get_onboarding_completion_rate(
    ...
    try:
        ...
        logger.error(
            f"Failed to get completion rate for user {current_user['user_id']}: {e!s}",
            exc_info=True,
        )
        raise HTTPException(status_code=500, detail="無法取得完成率，請稍後再試")
    except Exception as e:
        logger.error(
            f"Unexpected error getting completion rate for user {current_user['user_id']}: {e!s}",
            exc_info=True,
        )
        raise HTTPException(status_code=500, detail="系統錯誤，請稍後再試")


@router.get("/analytics/onboarding/drop-off", response_model=SuccessResponse[DropOffRatesResponse])
async def get_drop_off_rates(current_user: dict[str, Any] = Depends(get_current_user)):
    """
    ...
    try:
        ...
        logger.error(
            f"Failed to get drop-off rates for user {current_user['user_id']}: {e!s}",
            exc_info=True,
        )
        raise HTTPException(status_code=500, detail="無法取得流失率，請稍後再試")
    except Exception as e:
        logger.error(
            f"Unexpected error getting drop-off rates for user {current_user['user_id']}: {e!s}",
            exc_info=True,
        )
        raise HTTPException(status_code=500, detail="系統錯誤，請稍後再試")
...
@router.get(
    "/analytics/onboarding/average-time", response_model=SuccessResponse[AverageTimePerStepResponse]
)
async def get_average_time_per_step(current_user: dict[str, Any] = Depends(get_current_user)):
    """
    ...
    try:
        ...
        logger.error(
            f"Failed to get average time per step for user {current_user['user_id']}: {e!s}",
            exc_info=True,
        )
        raise HTTPException(status_code=500, detail="無法取得平均時間，請稍後再試")
    except Exception as e:
        logger.error(
            f"Unexpected error getting average time per step for user {current_user['user_id']}: {e!s}",
            exc_info=True,
        )
        raise HTTPException(status_code=500, detail="系統錯誤，請稍後再試")
