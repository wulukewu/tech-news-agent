...
async def add_to_reading_list(
    request: AddToReadingListRequest, current_user: dict = Depends(get_current_user)
):
    """
    加入文章到閱讀清單
    ...
    try:
        ...
        # 先驗證文章是否存在
        article_check = (
            supabase.client.table("articles").select("id").eq("id", str(article_id)).execute()
        )
        ...
        # 驗證錯誤（無效的 UUID 格式）
        logger.warning(
            f"Invalid article_id format: {e}", extra={"article_id": str(request.article_id)}
        )
...
@router.get("/reading-list", response_model=PaginatedResponse[ReadingListItemResponse])
    ...
    try:
        ...
        query = (
            ...
            .select(
                ...
                "articles!inner(id, title, url, tinkering_index, ai_summary, actionable_takeaway, feeds!inner(category))",
                count="exact",
            )
        ...
        # 執行查詢
        response = (
            query.order("added_at", desc=True).range(offset, offset + page_size - 1).execute()
        )
        ...
        for item_data in response.data:
            try:
                ...
                logger.warning(
                    f"Failed to parse reading list item: {e}", extra={"item_data": item_data}
                )
                continue
        ...
        logger.info(
            "Successfully fetched reading list",
            extra={
                "discord_id": discord_id,
        ...
        # 驗證錯誤
        logger.warning(f"Validation error: {e}", extra={"status": status})
        raise HTTPException(status_code=400, detail=str(e))
    except SupabaseServiceError as e:
        # 資料庫錯誤
        logger.error(
            ...
            exc_info=True,
            extra={"discord_id": current_user.get("discord_id"), "page": page, "status": status},
        )
        raise HTTPException(status_code=500, detail="Failed to fetch reading list")
    except Exception as e:
        # 未預期的錯誤
        logger.error(
...
@router.delete("/reading-list/{article_id}", response_model=SuccessResponse[MessageResponse])
async def remove_from_reading_list(
    article_id: UUID, current_user: dict = Depends(get_current_user)
):
    """
    從閱讀清單移除文章
    ...
    try:
        ...
        logger.info(
            "Removing article from reading list",
            extra={"discord_id": discord_id, "article_id": str(article_id)},
        )
        ...
        if not response.data or len(response.data) == 0:
            raise HTTPException(
                status_code=404, detail=f"Reading list entry not found for article: {article_id}"
            )

        logger.info(
            "Successfully removed article from reading list",
            extra={"discord_id": discord_id, "article_id": str(article_id)},
        )

        return success_response(
            MessageResponse(message="Article removed from reading list", article_id=article_id)
        )
        ...
        # 驗證錯誤（無效的 UUID 格式）
        logger.warning(f"Invalid article_id format: {e}", extra={"article_id": str(article_id)})
        raise HTTPException(status_code=400, detail=str(e))
    except SupabaseServiceError as e:
        # 資料庫錯誤
        logger.error(
            ...
            exc_info=True,
            extra={"discord_id": current_user.get("discord_id"), "article_id": str(article_id)},
        )
...
@router.patch("/reading-list/{article_id}/status", response_model=SuccessResponse[MessageResponse])
async def update_reading_list_status(
    article_id: UUID, request: UpdateStatusRequest, current_user: dict = Depends(get_current_user)
):
    """
    更新閱讀清單項目狀態
    ...
    try:
        ...
        logger.info(
            "Successfully updated reading list status",
            extra={
                "discord_id": discord_id,
                "article_id": str(article_id),
                "new_status": request.status,
...
@router.patch("/reading-list/{article_id}/rating", response_model=SuccessResponse[MessageResponse])
async def update_reading_list_rating(
    article_id: UUID, request: UpdateRatingRequest, current_user: dict = Depends(get_current_user)
):
    """
    更新閱讀清單項目評分
    ...
    try:
        ...
        logger.info(
            "Successfully updated reading list rating",
            extra={
                "discord_id": discord_id,
                "article_id": str(article_id),
                "new_rating": request.rating if request.rating is not None else "null",
        ...
        return success_response(
            MessageResponse(
                message="Rating updated successfully", article_id=article_id, rating=request.rating
            )
        )
