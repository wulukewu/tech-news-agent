...
import httpx
from fastapi import APIRouter, HTTPException, Response, Query, Header, Cookie, Depends
...
async def discord_login():
    ...
    if not settings.discord_redirect_uri:
        raise HTTPException(
            status_code=500,
            detail="DISCORD_REDIRECT_URI is not configured"
        )
...
async def discord_callback(
    ...
    if not settings.discord_client_id or not settings.discord_client_secret:
        raise HTTPException(
            status_code=500,
            detail="Discord OAuth2 configuration is incomplete"
        )
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail="Failed to communicate with Discord API"
        )
