...
async def get_onboarding_status(
    ...
    try:
        ...
        logger.error(
            f"Failed to get onboarding status for user {current_user['user_id']}: {str(e)}",
            exc_info=True
        )
        raise HTTPException(
            status_code=500,
            detail="無法取得引導進度，請稍後再試"
        )
    except Exception as e:
        logger.error(
            f"Unexpected error getting onboarding status for user {current_user['user_id']}: {str(e)}",
            exc_info=True
        )
        raise HTTPException(
            status_code=500,
...
async def update_onboarding_progress(
    ...
    try:
        ...
        return {"message": "引導進度已更新"}
        
    except OnboardingServiceError as e:
        logger.error(
            f"Failed to update onboarding progress for user {current_user['user_id']}: {str(e)}",
            exc_info=True
        )
        raise HTTPException(
            status_code=500,
            detail="無法更新引導進度，請稍後再試"
        )
    except Exception as e:
        logger.error(
            f"Unexpected error updating onboarding progress for user {current_user['user_id']}: {str(e)}",
            exc_info=True
        )
        raise HTTPException(
            status_code=500,
...
async def mark_onboarding_completed(
    ...
    try:
        ...
        return {"message": "引導已完成"}
        
    except OnboardingServiceError as e:
        logger.error(
            f"Failed to mark onboarding completed for user {current_user['user_id']}: {str(e)}",
            exc_info=True
        )
        raise HTTPException(
            status_code=500,
        ...
        logger.error(
            f"Unexpected error marking onboarding completed for user {current_user['user_id']}: {str(e)}",
            exc_info=True
        )
        raise HTTPException(
            status_code=500,
...
async def mark_onboarding_skipped(
    ...
    try:
        ...
        return {"message": "已跳過引導"}
        
    except OnboardingServiceError as e:
        logger.error(
            f"Failed to mark onboarding skipped for user {current_user['user_id']}: {str(e)}",
            exc_info=True
        )
        raise HTTPException(
            status_code=500,
        ...
        logger.error(
            f"Unexpected error marking onboarding skipped for user {current_user['user_id']}: {str(e)}",
            exc_info=True
        )
        raise HTTPException(
            status_code=500,
...
async def reset_onboarding(
    ...
    try:
        ...
        return {"message": "引導已重置"}
        
    except OnboardingServiceError as e:
        logger.error(
            f"Failed to reset onboarding for user {current_user['user_id']}: {str(e)}",
            exc_info=True
        )
        raise HTTPException(
            status_code=500,
        ...
        logger.error(
            f"Unexpected error resetting onboarding for user {current_user['user_id']}: {str(e)}",
            exc_info=True
        )
        raise HTTPException(
            status_code=500,
