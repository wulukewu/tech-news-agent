...
class AdminCommands(commands.Cog):
    ...
    async def trigger_fetch(self, interaction: discord.Interaction):
        ...
        try:
            ...
            logger.info("Scheduler triggered successfully", user_id=str(interaction.user.id))

        except Exception as e:
            logger.critical(
                "Failed to trigger scheduler manually",
                user_id=str(interaction.user.id),
                command="trigger_fetch",
                error=str(e),
    ...
    @app_commands.command(name="scheduler_status", description="查看 scheduler 的執行狀態")
    async def scheduler_status(self, interaction: discord.Interaction):
        ...
        try:
            ...
            if health_data["is_healthy"]:
                embed = discord.Embed(title="✅ Scheduler 狀態正常", color=discord.Color.green())
            else:
                embed = discord.Embed(title="⚠️ Scheduler 狀態異常", color=discord.Color.orange())

                # Add issues if any
                if health_data["issues"]:
                    issues_text = "\n".join(f"• {issue}" for issue in health_data["issues"])
                    embed.add_field(name="問題", value=issues_text, inline=False)
            ...
            if health_data["total_operations"] > 0:
                success_rate = (
                    (health_data["total_operations"] - health_data["failed_operations"])
                    / health_data["total_operations"]
                    * 100
                )
            ...
            await interaction.followup.send(embed=embed, ephemeral=True)
            logger.info(
                "Scheduler status sent successfully",
                user_id=str(interaction.user.id),
