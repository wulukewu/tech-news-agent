...
from app.tasks.scheduler import setup_scheduler, background_fetch_job, get_scheduler_health, scheduler, _scheduler_health
...
class TestSchedulerEmptyFeedHandling:
    ...
    async def test_empty_feed_list_handling(self, caplog):
        ...
        assert any("No active feeds found" in record.message 
...
class TestSchedulerDatabaseFailureHandling:
    ...
    async def test_scheduler_continues_after_database_failure(self, caplog):
        ...
        assert any("execution completed" in record.message.lower() and
...
class TestSchedulerDiscordDecoupling:
    ...
    def test_scheduler_does_not_import_discord_client(self):
        ...
        assert 'bot' not in module_dict, \
            "Scheduler should not import Discord bot"
        
        # Check that Discord-related classes are not imported
        assert 'MarkReadView' not in module_dict, \
            "Scheduler should not import MarkReadView"
        assert 'ReadLaterView' not in module_dict, \
            "Scheduler should not import ReadLaterView"
        
        # Check that discord module is not imported
        assert 'discord' not in module_dict, \
            "Scheduler should not import discord module"
...
class TestSchedulerRobustness:
    ...
    async def test_scheduler_does_not_crash_on_unexpected_exception(self, caplog):
        ...
        assert any("Unexpected error" in record.message 
                   for record in caplog.records if record.levelno == logging.CRITICAL)
    ...
    async def test_scheduler_logs_full_stack_trace_on_error(self, caplog):
        ...
        mock_supabase = MagicMock()
        mock_supabase.get_active_feeds = AsyncMock(
            side_effect=SupabaseServiceError("Test error")
        )
