...
async def test_property_8_batch_result_accuracy(
    ...
    assert captured_batch_result is not None, \
        "Batch result should be returned from insert_articles"
...
async def test_property_8_multiple_batches_accuracy(num_articles, failure_rate):
    ...
    assert len(all_batch_results) > 0, \
    ...
    for i, batch_result in enumerate(all_batch_results):
        ...
        assert batch_total > 0, \
            f"Batch {i} should have processed at least one article"
...
async def test_property_8_all_success_accuracy(num_articles):
    ...
    assert captured_batch_result.failed_count == 0, \
        "Failed count should be 0 when all operations succeed"
    
    assert len(captured_batch_result.failed_articles) == 0, \
    ...
    assert captured_batch_result.success_rate == 1.0, \
...
async def test_property_8_all_fail_accuracy(num_articles):
    ...
    with patch('app.tasks.scheduler.SupabaseService') as mock_supabase_class, \
        ...
        async def mock_insert_articles(articles_dict_list):
            ...
            failed_articles = [
                {'url': a['url'], 'error': 'Simulated failure'}
                for a in articles_dict_list
            ]
    ...
    assert captured_batch_result.inserted_count == 0, \
        "Inserted count should be 0 when all operations fail"
    
    assert captured_batch_result.updated_count == 0, \
    ...
    assert captured_batch_result.success_rate == 0.0, \
