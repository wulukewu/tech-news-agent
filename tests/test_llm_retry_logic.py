...
from app.services.llm_service import LLMService, MAX_RETRIES, RETRY_DELAYS
...
class TestRetryLogic:
    ...
    async def test_logs_retry_attempts(self, caplog):
        ...
        assert any("Retry attempt 1/2 for test_context" in record.message for record in caplog.records)
        assert any("Retry attempt 2/2 for test_context" in record.message for record in caplog.records)
    ...
    async def test_logs_final_error_on_exhaustion(self, caplog):
        ...
        assert any("All retry attempts exhausted for test_context" in record.message for record in caplog.records)
