...
async def test_property_12_article_time_filtering(
    ...
    for article in old_articles:
        assert str(article.url) not in result_urls, \
    ...
    assert len(result) == num_recent_articles, \
...
async def test_property_12_article_time_filtering_boundary(days_to_fetch):
    ...
    assert str(article_at_cutoff.url) in result_urls, \
        "Article at exact cutoff should be included"
    
    # Article after cutoff should be included
    assert str(article_after_cutoff.url) in result_urls, \
        "Article after cutoff should be included"
    
    # Article before cutoff should be filtered
    assert str(article_before_cutoff.url) not in result_urls, \
        "Article before cutoff should be filtered out"
    
    # Should have exactly 2 articles
    assert len(result) == 2, \
        f"Expected 2 articles (at and after cutoff), got {len(result)}"
