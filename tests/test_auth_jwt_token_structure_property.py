...
def test_jwt_token_structure_property(user_id, discord_id):
    ...
    **Validates: Requirements 3.1, 3.2, 3.3, 3.4, 3.5**
    ...
    assert payload["sub"] == str(user_id), \
        f"Payload 'sub' should match user_id: expected {str(user_id)}, got {payload['sub']}"
    ...
    assert abs(exp_delta_seconds - expected_delta_seconds) < 5, \
...
@given(
    ...
    expires_delta=st.one_of(
        ...
        st.builds(
            timedelta,
            seconds=st.integers(min_value=1, max_value=365 * 24 * 60 * 60)
        )
    )
)
