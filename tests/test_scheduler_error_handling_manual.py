...
class TestSchedulerErrorHandling:
    ...
    async def test_scheduler_logs_full_stack_trace_on_error(self, caplog):
        ...
        with patch('app.tasks.scheduler.SupabaseService') as mock_supabase_class:
            ...
            critical_messages = [record.message for record in caplog.records if record.levelname == "CRITICAL"]
            assert any("All database retry attempts failed" in msg or "Database error during background fetch job" in msg 
                      for msg in critical_messages), f"Expected critical error log, got: {critical_messages}"
    ...
    async def test_scheduler_skips_execution_after_all_retries_fail(self, caplog):
        ...
        with patch('app.tasks.scheduler.SupabaseService') as mock_supabase_class:
            ...
            assert any("Skipping current job execution" in record.message 
                      for record in caplog.records if record.levelname == "CRITICAL")
    ...
    async def test_scheduler_continues_to_next_execution_after_failure(self, caplog):
        ...
        with patch('app.tasks.scheduler.SupabaseService') as mock_supabase_class:
            ...
            assert any("Scheduler will continue to next scheduled execution" in record.message 
    ...
    async def test_scheduler_handles_unexpected_exceptions(self, caplog):
        ...
        with patch('app.tasks.scheduler.SupabaseService') as mock_supabase_class:
            ...
            assert any("Unexpected error during background fetch job" in record.message 
                      for record in caplog.records if record.levelname == "CRITICAL")
    ...
    async def test_scheduler_retries_article_insertion_on_failure(self):
        ...
        with patch('app.tasks.scheduler.SupabaseService') as mock_supabase_class, \
            ...
            mock_llm = MagicMock()
            mock_llm.evaluate_batch = AsyncMock(return_value=[
                MagicMock(
                    title="Test Article",
                    url="https://example.com/article",
                    feed_id="test-feed-id",
            ...
            assert insertion_attempts == 3, f"Expected 3 insertion attempts, got {insertion_attempts}"
