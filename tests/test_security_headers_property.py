...
def test_security_headers_presence_property(client, endpoint, method):
    ...
    **Validates: Requirements 18.5**
    ...
    x_content_type_options = (
        response.headers.get("X-Content-Type-Options") or
    ...
    x_frame_options = (
        response.headers.get("X-Frame-Options") or
        response.headers.get("x-frame-options")
    )
    assert x_frame_options is not None, \
        f"X-Frame-Options header missing for {method} {endpoint}"
    assert x_frame_options.upper() == "DENY", \
        f"X-Frame-Options should be 'DENY', got: {x_frame_options}"
    
    # Verify X-XSS-Protection header
    x_xss_protection = (
        response.headers.get("X-XSS-Protection") or
        response.headers.get("x-xss-protection")
    )
    assert x_xss_protection is not None, \
        f"X-XSS-Protection header missing for {method} {endpoint}"
    assert "1" in x_xss_protection and "mode=block" in x_xss_protection.lower(), \
...
def test_security_headers_all_endpoints(client):
    ...
    **Validates: Requirements 18.5**
    ...
    for endpoint in endpoints:
        ...
        assert "X-XSS-Protection" in response.headers or \
...
def test_security_headers_correct_values(client):
    ...
    **Validates: Requirements 18.5**
    ...
    assert x_frame == "DENY", \
        f"X-Frame-Options should be 'DENY', got: {x_frame}"
    ...
    assert x_xss == "1; mode=block", \
        f"X-XSS-Protection should be '1; mode=block', got: {x_xss}"
...
@given(
    status_code=st.sampled_from([200, 400, 401, 404, 500])
)
def test_security_headers_on_various_status_codes(client, status_code):
    ...
    **Validates: Requirements 18.5**
    ...
    has_xss_protection = (
        "X-XSS-Protection" in response.headers or
        "x-xss-protection" in response.headers
    )
    
    assert has_content_type_options, \
