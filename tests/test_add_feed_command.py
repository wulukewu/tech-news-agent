...
async def test_add_feed_new_feed_new_subscription(subscription_cog, mock_interaction):
    ...
    with patch('app.bot.cogs.subscription_commands.ensure_user_registered') as mock_ensure_user, \
        ...
        mock_service.client.table.return_value.select.return_value.eq.return_value.execute.return_value = Mock(data=[])
...
async def test_add_feed_existing_feed_new_subscription(subscription_cog, mock_interaction):
    ...
    with patch('app.bot.cogs.subscription_commands.ensure_user_registered') as mock_ensure_user, \
        ...
        # Act
        await subscription_cog.add_feed.callback(subscription_cog, 
            mock_interaction,
            name="Existing Feed",
            url="https://example.com/feed.xml",
            category="Web"
...
async def test_add_feed_database_error(subscription_cog, mock_interaction):
    ...
    with patch('app.bot.cogs.subscription_commands.ensure_user_registered') as mock_ensure_user, \
        ...
        # Mock database error
        mock_service.client.table.return_value.select.return_value.eq.return_value.execute.side_effect = \
            SupabaseServiceError("Database connection failed")
...
async def test_add_feed_feed_creation_failure(subscription_cog, mock_interaction):
    ...
    with patch('app.bot.cogs.subscription_commands.ensure_user_registered') as mock_ensure_user, \
        ...
        mock_service.client.table.return_value.insert.return_value.execute.return_value = Mock(data=[])
...
@pytest.mark.asyncio
async def test_add_feed_validates_url_before_database_operations(subscription_cog, mock_interaction):
    """
