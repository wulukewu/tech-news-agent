...
def test_property_27_exception_wrapping_preserves_original_error(
    ...
    **Validates: Requirements 13.8**
    ...
    if original_error is not None:
        ...
        assert str(original_error) in error_str, "Error string should contain original error details"
...
@given(
    message=exception_messages(),
    original_error=original_exceptions()
)
@settings(max_examples=5, deadline=None)
def test_property_27_exception_wrapping_maintains_exception_chain(
    ...
    **Validates: Requirements 13.8**
    ...
    assert wrapped_error.original_error is original_error, "Original error should be the same object"
    
    # Verify original error type is preserved
    assert type(wrapped_error.original_error) == type(original_error), "Original error type should be preserved"
    
    # Verify original error message is preserved
    assert str(wrapped_error.original_error) == str(original_error), "Original error message should be preserved"
    ...
    assert caught_error.original_error is original_error, "Caught error should have the same original error"
...
@settings(max_examples=5, deadline=None)
def test_property_27_exception_wrapping_stores_context_information(
    message: str,
    context: dict
):
    """
...
@settings(max_examples=5, deadline=None)
def test_property_27_exception_wrapping_handles_missing_context(
    message: str
):
    """
    **Validates: Requirements 13.8**
    ...
    error_with_none = SupabaseServiceError(
        message=message,
        original_error=None,
        context=None
    )
    ...
    assert isinstance(error_without_context.context, dict), "Context should be a dict even when omitted"
...
def test_property_27_exception_wrapping_string_representation_format(
    ...
    **Validates: Requirements 13.8**
    ...
    if expected_components > 1:
        ...
        assert len(components) == expected_components, f"Should have {expected_components} components"
