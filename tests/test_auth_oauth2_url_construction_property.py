...
@given(
    ...
    redirect_uri=valid_redirect_uri_strategy()
)
def test_oauth2_url_construction_property(client_id, redirect_uri):
    ...
    **Validates: Requirements 1.2, 1.3, 1.4, 1.5**
    ...
    assert actual_base == expected_base, \
        f"Base URL should be {expected_base}, got {actual_base}"
    ...
    for param in required_params:
        assert param in query_params, \
            f"Required parameter '{param}' is missing from URL"
        assert len(query_params[param]) > 0, \
            f"Parameter '{param}' should have at least one value"
...
def test_oauth2_url_construction_idempotence_property(client_id, redirect_uri):
    ...
    **Validates: Requirements 1.2, 1.3, 1.4, 1.5**
    ...
    assert url1 == url2, \
        f"URL construction should be idempotent: first={url1}, second={url2}"
    ...
    assert params1 == params2, \
        f"Query parameters should match: first={params1}, second={params2}"
...
def test_oauth2_url_construction_encoding_property(client_id, redirect_uri):
    ...
    **Validates: Requirements 1.2, 1.3, 1.4, 1.5**
    ...
    assert decoded_redirect_uri == redirect_uri, \
...
def test_oauth2_url_construction_no_extra_parameters_property(client_id, redirect_uri):
    ...
    **Validates: Requirements 1.2, 1.3, 1.4, 1.5**
    ...
    assert len(extra_params) == 0, \
        f"URL should not contain extra parameters: {extra_params}"
    ...
    # Verify exact match
    assert actual_params == required_params, \
