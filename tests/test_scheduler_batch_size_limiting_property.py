...
async def test_property_14_batch_size_limiting_over_50(num_articles):
    ...
    assert len(batch_sizes) > 0, \
        "At least one batch should be created"
    ...
    assert len(batch_sizes) == expected_num_batches, \
...
@given(
    num_articles=st.integers(min_value=101, max_value=300)
)
@pytest.mark.asyncio
async def test_property_14_batch_size_limiting_over_100(num_articles):
    ...
    for i, batch_size in enumerate(batch_sizes):
        assert batch_size <= 50, \
            f"Batch {i + 1} has {batch_size} articles, exceeds limit of 50"
    
    # Property 3: All batches except the last should have exactly 50 articles
    for i, batch_size in enumerate(batch_sizes[:-1]):
        assert batch_size == 50, \
            f"Batch {i + 1} (not last) has {batch_size} articles, expected 50"
    ...
    assert batch_calls == len(batch_sizes), \
...
async def test_property_14_batch_size_limiting_exactly_100():
    ...
    assert batch_sizes[0] == 50, \
        f"First batch should contain 50 articles, got {batch_sizes[0]}"
    assert batch_sizes[1] == 50, \
        f"Second batch should contain 50 articles, got {batch_sizes[1]}"
    
    # Property 3: Total should equal 100
    assert sum(batch_sizes) == 100, \
        f"Total articles should be 100, got {sum(batch_sizes)}"
...
async def test_property_14_batch_size_limiting_51_articles():
    ...
    # Property 1: Exactly 2 batches should be created
    assert len(batch_sizes) == 2, \
        f"For 51 articles, expected 2 batches, got {len(batch_sizes)}"
    ...
    assert batch_sizes[1] == 1, \
        f"Second batch should contain 1 article, got {batch_sizes[1]}"
    
    # Property 4: Total should equal 51
    assert sum(batch_sizes) == 51, \
        f"Total articles should be 51, got {sum(batch_sizes)}"
