...
async def test_property_2_article_deduplication_correctness(
    ...
    for url in result_urls:
        assert url not in existing_urls, \
    ...
    for url in new_urls:
        assert url in result_urls, \
            f"Output missing new URL {url}, should be included"
    ...
    assert len(result_urls) == len(result), \
        "Output contains duplicate articles"
...
async def test_property_2_all_articles_new(num_articles):
    ...
    for article in fetched_articles:
        assert str(article.url) in result_urls, \
            f"Article {article.url} should be in result"
...
async def test_property_2_all_articles_existing(num_articles):
    ...
    assert len(result) == 0, \
        f"Expected 0 articles when all exist, got {len(result)}"
...
async def test_property_2_deduplication_with_check_failures(num_articles, num_check_failures):
    ...
    for url in failing_urls:
        assert url in result_urls, \
            f"URL {url} with failed check should be included (fail-safe)"
    
    # Property 2: New URLs should be included
    for url in new_urls:
        assert url in result_urls, \
            f"New URL {url} should be included"
    
    # Property 3: Existing URLs should NOT be included
    for url in existing_urls:
        assert url not in result_urls, \
            f"Existing URL {url} should be filtered out"
...
@given(
    num_existing_in_db=st.integers(min_value=0, max_value=100)
)
@pytest.mark.asyncio
async def test_property_2_empty_fetch_result(num_existing_in_db):
    ...
    assert len(result) == 0, \
        f"Expected empty result when no articles fetched, got {len(result)}"
    
    # Property: check_article_exists should not be called
    assert mock_supabase.check_article_exists.call_count == 0, \
