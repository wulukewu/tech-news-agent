...
class TestDiscordMultiTenantE2E:
    ...
    async def test_multi_user_isolation_e2e(
        ...
        **Validates: Requirements 3.1-3.5, 6.1-6.8, 9.1-9.9**
        ...
        feed_b_result = test_supabase_client.table('feeds').insert({
            'name': 'User B Feed',
            'url': feed_b_url,
        ...
        # Get article IDs
        response_a = test_supabase_client.table('articles').select('id').eq('url', articles_a[0]['url']).execute()
        article_a_id = response_a.data[0]['id']
        cleanup_test_data['articles'].append(article_a_id)
        
        response_b = test_supabase_client.table('articles').select('id').eq('url', articles_b[0]['url']).execute()
        ...
        response = test_supabase_client.table('articles')\
            .select('id, title')\
    ...
    async def test_performance_response_times(
        ...
        **Validates: Requirements 15.1-15.2**
        ...
        response = test_supabase_client.table('articles').select('id').eq('url', articles[0]['url']).execute()
        article_id = response.data[0]['id']
        ...
        assert reading_list_time < 2.0, f"/reading_list view took {reading_list_time:.2f}s (should be < 2s)"
