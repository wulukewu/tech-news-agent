...
async def test_property_36_context_manager_resource_cleanup(should_raise_exception, exception_type):
    ...
    **Validates: Requirements 17.3**
    ...
    if should_raise_exception:
        ...
        assert close_called, f"close() should be called even when {exception_type.__name__} is raised"
...
@given(
    num_operations=st.integers(min_value=0, max_value=10)
)
@settings(max_examples=5, suppress_health_check=[HealthCheck.function_scoped_fixture], deadline=None)
@pytest.mark.asyncio
...
async def test_property_36_context_manager_cleanup_with_various_exceptions(exception_message):
    ...
    **Validates: Requirements 17.3**
    ...
    assert close_called, f"close() should be called even with exception message: {exception_message[:50]}"
...
async def test_property_36_context_manager_nested_contexts(nested_level):
    ...
    **Validates: Requirements 17.3**
    ...
    assert len(close_calls) == nested_level, \
...
async def test_property_36_context_manager_vs_manual_close(use_context_manager):
    ...
    **Validates: Requirements 17.3**
    ...
    assert close_called, \
        f"close() should be called with use_context_manager={use_context_manager}"


@given(
    has_exception=st.booleans()
)
@settings(max_examples=5, suppress_health_check=[HealthCheck.function_scoped_fixture], deadline=None)
@pytest.mark.asyncio
