...
@given(
    unanalyzed_articles=st.lists(
        article_state_strategy(),
        min_size=1,
        max_size=20
    )
)
...
async def test_property_15_reprocessing_null_ai_summary(unanalyzed_articles):
    ...
    assert mock_supabase.insert_articles.called, \
...
@given(
    num_complete_articles=st.integers(min_value=1, max_value=10)
)
@pytest.mark.asyncio
async def test_property_15_skip_complete_articles(num_complete_articles):
    ...
    assert not llm_called_for_reprocessing, \
...
@given(
    num_partial_articles=st.integers(min_value=1, max_value=10)
)
@pytest.mark.asyncio
async def test_property_15_selective_reprocessing(num_partial_articles):
    ...
    with patch('app.tasks.scheduler.SupabaseService') as mock_supabase_class, \
        ...
        mock_supabase.insert_articles.return_value = BatchResult(
            inserted_count=0,
            updated_count=num_partial_articles,
            failed_count=0,
            failed_articles=[]
        )
    ...
    assert reprocessed_count == num_partial_articles, \
    ...
    assert mock_llm.evaluate_batch.called, \
...
async def test_property_15_timestamp_update_on_reprocessing(num_articles):
    ...
    assert len(inserted_articles) == num_articles, \
    ...
    for article_dict in inserted_articles:
        assert 'url' in article_dict, \
            "Article must have 'url' field for UPSERT operation"
        assert 'title' in article_dict, \
            "Article must have 'title' field"
        assert 'feed_id' in article_dict, \
            "Article must have 'feed_id' field"
        assert 'ai_summary' in article_dict, \
            "Article must have 'ai_summary' field after re-processing"
        assert 'tinkering_index' in article_dict, \
            "Article must have 'tinkering_index' field after re-processing"
        
        # Verify that ai_summary and tinkering_index are not NULL
        assert article_dict['ai_summary'] is not None, \
            "Re-processed article should have non-NULL ai_summary"
        assert article_dict['tinkering_index'] is not None, \
...
async def test_property_15_mixed_article_states(articles):
    ...
    with patch('app.tasks.scheduler.SupabaseService') as mock_supabase_class, \
        ...
        async def mock_evaluate_batch(batch_articles):
            ...
            for article in batch_articles:
                reprocessed_urls.add(str(article.url))
                article.tinkering_index = 3
                article.ai_summary = f"Re-processed summary"
                result.append(article)
            return result
    ...
    assert len(reprocessed_urls) == len(needs_reprocessing), \
