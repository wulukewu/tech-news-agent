...
def make_source(name: str = "TestFeed", url: str = "https://example.com/feed.xml", category: str = "AI") -> RSSSource:
...
class TestFetchNewArticles:
    ...
    async def test_returns_only_new_articles(self):
        ...
        with patch.object(service, 'fetch_all_feeds', AsyncMock(return_value=[article1, article2, article3])):
            # Mock supabase service: article2 exists, others don't
            mock_supabase = AsyncMock()
            mock_supabase.check_article_exists = AsyncMock(side_effect=lambda url: url == "https://example.com/article2")
